...
stse_ReturnCode_t stsafel_ecc_generate_signature(stse_Handler_t *pSTSE,
						 stse_ecc_key_type_t key_type, PLAT_UI8 *pChallenge,
						 PLAT_UI16 challenge_length, PLAT_UI8 *pSignature)
{
	...
	if ((key_type != STSE_ECC_KT_ED25519) ||
	    (challenge_length != STSAFEL_ECC_SIGNATURE_CHALLENGE_LENGTH) || (pChallenge == NULL) ||
	    (pSignature == NULL)) {
	...
	stse_frame_element_allocate_push(&CmdFrame, eInternal_data_subject, 1,
					 &internal_data_subject);

	/*- Create Rsp frame and populate elements*/
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEL_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eSignature,
					 stse_ecc_info_table[key_type].signature_size, pSignature);
