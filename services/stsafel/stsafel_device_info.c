...
{
	return stsafel_get_data(pSTSE, STSAFEL_INFO_UNIQUE_IDENTIFIER, 0, NULL, STSAFEL_UID_LENGTH,
				pDevice_UID);
}
...
{
	return stsafel_get_data(pSTSE, STSAFEL_INFO_TRACEABILITY_DATA, 0, NULL,
				   ...
				   PLAT_UI16 device_info_expected_length, PLAT_UI8 *pDevice_info)
{
	...
	if ((pAdditional_data == NULL && additional_data_length != 0) || (pDevice_info == NULL)) {
	...
	stse_frame_element_allocate_push(&CmdFrame, eAdditional_data, additional_data_length,
					 pAdditional_data);

	/*- Create Rsp frame and populate elements*/
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEL_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eDevice_info, device_info_expected_length,
					 pDevice_info);
