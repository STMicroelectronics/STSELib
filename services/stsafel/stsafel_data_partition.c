					 ...
					 stse_cmd_protection_t protection)
{
	...
	stse_frame_element_allocate_push(&CmdFrame, eRead_option, sizeof(stsafel_read_option_t),
					 (PLAT_UI8 *)&option);
					   ...
					   stse_cmd_protection_t protection)
{
	...
	stse_frame_allocate(CmdFrame);
	stse_frame_element_allocate_push(&CmdFrame, eCmd_header, STSAFEL_HEADER_SIZE, &cmd_header);
	stse_frame_element_allocate_push(&CmdFrame, eUpdate_option, sizeof(stsafel_update_option_t),
					 (PLAT_UI8 *)&option);
	stse_frame_element_allocate_push(&CmdFrame, eZone_index, 1, &zone_index);
	stse_frame_element_allocate_push(&CmdFrame, eOffset, 2, (PLAT_UI8 *)&offset);
					    ...
					    stse_cmd_protection_t protection)
{
	...
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEL_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eCounter, STSAFEL_COUNTER_VALUE_SIZE,
					 temp_counter);
	stse_frame_element_allocate_push(&RspFrame, eData, data_length, pData);
	...
	/*- Perform Transfer*/
	ret = stsafel_frame_transfer(pSTSE, &CmdFrame, &RspFrame);

	if (ret == STSE_OK) {
		*pCounter_value =
			((temp_counter[2]) | (temp_counter[1] << 8) | (temp_counter[0] << 16));
	}
						 ...
						 stse_cmd_protection_t protection)
{
	...
	PLAT_UI8 decrement_amount[STSAFEL_COUNTER_VALUE_SIZE] = {
		((amount & 0xFF0000) >> 16), ((amount & 0xFF00) >> 8), (amount & 0xFF)};
	PLAT_UI8 temp_counter[STSAFEL_COUNTER_VALUE_SIZE];
	...
	stse_frame_allocate(CmdFrame);
	stse_frame_element_allocate_push(&CmdFrame, eCmd_header, STSAFEL_HEADER_SIZE, &cmd_header);
	stse_frame_element_allocate_push(&CmdFrame, eDecrement_option,
					 sizeof(stsafel_decrement_option_t), (PLAT_UI8 *)&option);
	stse_frame_element_allocate_push(&CmdFrame, eZone_index, 1, &zone_index);
	stse_frame_element_allocate_push(&CmdFrame, eOffset, 2, (PLAT_UI8 *)&offset);
	stse_frame_element_allocate_push(&CmdFrame, eAmount, STSAFEL_COUNTER_VALUE_SIZE,
					 decrement_amount);
	stse_frame_element_allocate_push(&CmdFrame, eData, data_length, pData);
