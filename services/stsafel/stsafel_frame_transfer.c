...
stse_ReturnCode_t stsafel_frame_transmit(stse_Handler_t *pSTSE, stse_frame_t *pFrame)
{
  ...
  /* STSAFE-L transmit follows the same linked-element streaming model as STSAFE-A:
   * compute CRC on logical frame, append temporary CRC element, then stream. */
  ...
  {
    /* Reject oversize frame before any bus transaction to avoid partial writes. */
  ...
  {
    /* Start/continue/stop callbacks let platform layer optimize bus transfers. */
    ...
    if (ret != STSE_OK)
    {
      /* Retry handles bus arbitration loss or temporary peripheral busy windows. */
      retry_count--;
...
stse_ReturnCode_t stsafel_i2c_frame_receive(stse_Handler_t *pSTSE, stse_frame_t *pFrame)
{
  ...
  /* I2C receive performs an initial length probe, then a second pass that maps
   * payload bytes across the caller-provided frame element chain. */
  ...
  {
    /* Length prefetch itself can carry protocol status on some transport adapters. */
    return ret;
  }
  ...
  /* Reported length includes CRC trailer; subtract it for payload accounting. */
  ...
  if (received_length > pFrame->length)
  {
    ...
    /* Filler element absorbs unread bytes when local frame map is shorter. */
  ...
  /* Unified receive loop reduces branching when dynamic frame sizes vary. */
  ...
  {
    /* Keep only status header element for error responses. */
  ...
  /* CRC trailer is captured as a transient frame element and validated later. */
  ...
  while (pCurrent_element != pFrame->last_element)
  {
    /* Clamp each element length to remaining bytes to support variable-size responses. */
  ...
  if (ret != STSE_OK)
  {
    /* Stop phase reads trailing element (CRC) and closes transaction atomically. */
    /* - Pop CRC element from Frame*/
    stse_frame_pop_element(pFrame);
    /* - Pop Filler element from Frame*/
    if (filler_size > 0)
  ...
  /* - Swap CRC */
  /* Convert from wire byte order for scalar compare with computed CRC16. */
  ...
  {
    /* CRC mismatch indicates transport corruption, not command-level STSAFE status. */
...
stse_ReturnCode_t stsafel_st1wire_frame_receive(stse_Handler_t *pSTSE, stse_frame_t *pFrame)
{
  ...
  /* ST1WIRE mode receives a fixed-length response defined by prepared frame shape,
   * so no filler element is needed in this path. */
  ...
  /* - Append CRC element to the RSP Frame (valid only in Receive Scope) */
  /* CRC element is appended before reception so BusRecvStop can target tail bytes. */
  STSE_FRAME_ELEMENT_ALLOCATE_PUSH(pFrame, eCRC, STSE_FRAME_CRC_SIZE, received_crc);
  ...
  while ((retry_count != 0) && (ret == STSE_PLATFORM_BUS_ACK_ERROR))
  {
    /* ST1WIRE receive can require repeated polling while target finalizes command. */
    ret = pSTSE->io.BusRecvStart(
            pSTSE->io.busID,
            pSTSE->io.Devaddr,
            pSTSE->io.BusSpeed,
  ...
  if (ret != STSE_OK)
  {
    /* Non-OK status aborts immediately without parsing remaining payload elements. */
    return ret;
  }
  ...
  while (pCurrent_element != pFrame->last_element)
  {
    /* Same defensive length clamp as I2C path for robust mixed element layouts. */
    if (received_length < pCurrent_element->length)
    {
      pCurrent_element->length = received_length;
    }
  ...
  stse_frame_debug_print(pFrame);
  printf("\n\r");
#endif /* defined(STSE_FRAME_DEBUG_LOG) */
                                             ...
                                             PLAT_UI16 inter_frame_delay)
{
  ...
  /* Raw transfer is bus-agnostic; receive helper is selected from runtime bus type. */
  ...
  if (ret == STSE_OK)
  {
    ...
    /* BusType switch dispatches to protocol-specific receive implementation. */
                                         ...
                                         stse_frame_t *pRspFrame)
{
  ...
  {
    /* First command byte selects execution-time table entry for wait scheduling. */
    if (pCmdFrame->first_element->length == 1) /* STSAFEA_HEADER_SIZE */
    {
      /* STSAFE-L command header is one byte; no extended header namespace in this path. */
