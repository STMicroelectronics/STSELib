					       ...
					       stse_cmd_authorization_record_t *pRecord_table)
{
	...
	PLAT_UI8
	raw_data[total_command_count * sizeof(stse_cmd_authorization_record_t)];
	...
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, 1, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eCR, sizeof(stse_cmd_authorization_CR_t),
					 (PLAT_UI8 *)pChange_rights);
	stse_frame_element_allocate_push(&RspFrame, eRecordCount, 1, &record_index);
	stse_frame_element_allocate_push(
		&RspFrame, eRecordTable,
		total_command_count * sizeof(stse_cmd_authorization_record_t), raw_data);
	...
	for (record_index = 0; record_index < total_command_count; record_index++) {
		...
		pRecord_table[record_index].command_AC =
			(stse_cmd_access_conditions_t)raw_data[record_array_pos++];
		pRecord_table[record_index].host_encryption_flags.cmd =
			(raw_data[record_array_pos] & 0x02) >> 1;
		pRecord_table[record_index].host_encryption_flags.rsp =
			(raw_data[record_array_pos++] & 0x01);
	}
...
stse_ReturnCode_t stsafea_perso_info_update(stse_Handler_t *pSTSE)
{
	...
	ret = stsafea_get_command_AC_table(pSTSE, total_command_count, &change_rights,
					   record_table);
	if (ret != STSE_OK) {
		return (ret);
	}
	...
	for (PLAT_UI8 i = 0; i < total_command_count; i++) {
		...
		if (record_table[i].extended_header == 0) {
			stsafea_perso_info_set_cmd_AC(pSTSE->pPerso_info, record_table[i].header,
						      record_table[i].command_AC);
			stsafea_perso_info_set_cmd_encrypt_flag(
				pSTSE->pPerso_info, record_table[i].header,
				record_table[i].host_encryption_flags.cmd);
			stsafea_perso_info_set_rsp_encrypt_flag(
				pSTSE->pPerso_info, record_table[i].header,
				record_table[i].host_encryption_flags.rsp);
		} else {
			stsafea_perso_info_set_ext_cmd_AC(pSTSE->pPerso_info,
							  record_table[i].extended_header,
							  record_table[i].command_AC);
			stsafea_perso_info_set_ext_cmd_encrypt_flag(
				pSTSE->pPerso_info, record_table[i].extended_header,
				record_table[i].host_encryption_flags.cmd);
			stsafea_perso_info_set_ext_rsp_encrypt_flag(
				pSTSE->pPerso_info, record_table[i].extended_header,
				record_table[i].host_encryption_flags.rsp);
		}
	}
...
{
	*pProtection = (stse_cmd_access_conditions_t)((pPerso->cmd_AC_status >>
...
void stsafea_perso_info_get_ext_cmd_AC(stse_perso_info_t *pPerso, PLAT_UI8 command_code,
				       stse_cmd_access_conditions_t *pProtection)
{
	*pProtection = (stse_cmd_access_conditions_t)((pPerso->ext_cmd_AC_status >>
						       (command_code + command_code)) &
						      0x03);
}
