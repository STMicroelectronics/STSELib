 ...
 * \param[in] 	slot_number 		Key slot in symmetric key table to be
 * used \param[in] 	message_length 		Length of the message \param[in]
 * pPlaintext_message	Plaintext message to encrypt \param[out]
 * pEncrypted_message	Encrypted message \return \ref STSE_OK on success ; \ref
 ...
 * \param[in] 	slot_number 		Key slot in symmetric key table to be
 * used \param[in] 	message_length 		Length of the message \param[in]
 * pEncrypted_message	Encrypted message to decrypt \param[out]
 * pPlaintext_message	Plaintext message \return \ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
 ...
 * Handler
 * \param[in] 	slot_number 					Key slot in
 * symmetric key table to be used \param[in] 	authentication_tag_length
 * Expected length for the authentication tag \param[in]	pNonce
 * Buffer containing the nonce
 * \param[in]	associated_data_length			Length of the associated
 ...
 * specified key from STSAFE symmetric key table \param[in] 	pSTSE
 * Pointer to STSE Handler \param[in] 	slot_number
 * Key slot in symmetric key table to be used \param[in]	Nonce_length
 * Nonce buffer length in bytes \param[in]	pNonce
 * Nonce buffer \param[in]	total_associated_data_length	Length of the
 * total amount of associated data \param[in]	total_message_length
 ...
 * be used \param[in] authentication_tag_length Expected length for the
 * authentication tag \param[in]	pNonce Buffer containing the nonce
 * \param[in] associated_data_length			Length of the associated
 * data \param[in]	pAssociated_data				Buffer
 * containing associated data
 * \param[in]	message_length					Length of the
 ...
 * containing the message to decrypt \param[in]	pAuthentication_tag Buffer
 * containing the authentication tag \param[out]	pVerification_result
 * Verification result flag \param[out]	pPlaintext_message
 * Buffer to store the decrypted message \return \ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
stse_ReturnCode_t stsafea_aes_ccm_decrypt(
	stse_Handler_t *pSTSE, PLAT_UI8 slot_number, PLAT_UI8 authentication_tag_length,
	PLAT_UI8 *pNonce, PLAT_UI16 associated_data_length, PLAT_UI8 *pAssociated_data,
	PLAT_UI16 message_length, PLAT_UI8 *pEncrypted_message, PLAT_UI8 *pAuthentication_tag,
	PLAT_UI8 *pVerification_result, PLAT_UI8 *pPlaintext_message);
 ...
 * Nonce buffer length in bytes \param[in]	pNonce
 * Nonce buffer \param[in]	total_associated_data_length	Length of the
 * associated data \param[in]	total_ciphertext_length			Length
 * of the complete ciphertext
 * \param[in]	associated_data_chunk_length	Length of the associated data
 * chunk \param[in]	pAssociated_data_chunk			Buffer
 * containing associated data chunk \param[in]	message_chunk_length
 ...
 * specified key from STSAFE symmetric key table \param[in] 	pSTSE
 * Pointer to STSE Handler \param[in] 	slot_number
 * Key slot in symmetric key table to be used \param[in]	IV_length
 * IV buffer length in bytes
 * \param[in]	pIV IV
 ...
 * error code otherwise
 */
stse_ReturnCode_t stsafea_aes_gcm_decrypt(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
					  PLAT_UI8 authentication_tag_length, PLAT_UI16 IV_length,
					  PLAT_UI8 *pIV, PLAT_UI16 associated_data_length,
					  PLAT_UI8 *pAssociated_data, PLAT_UI16 message_length,
					  PLAT_UI8 *pEncrypted_message,
