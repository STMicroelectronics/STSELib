...
/**
 * \brief 		Start chunk decryption in AES ECB mode
 * \details 	This service start chunk decryption in AES ECB mode using the specified key from STSAFE symmetric key table
 * \param[in] 	pSTSE 				Pointer to STSAFE Handler
 * \param[in] 	slot_number 		Key slot in symmetric key table to be used
 * \return \ref stse_ReturnCode_t : STSAFE_OK on success ; error code otherwise
 * \details 	\include{doc} stse_aes_ecb_encrypt.dox
 */
stse_ReturnCode_t stsafea_aes_ecb_decrypt_start(
		stse_Handler_t * pSTSE,
		PLAT_UI8 slot_number);

/**
 * \brief 		Process chunk decryption in AES ECB mode
 * \details 	This service process additional chunk decryption in AES ECB mode using the key specified in start command
 * \param[in] 	pSTSE 				Pointer to STSAFE Handler
 * \param[in] 	chunk_length 		Length of the chunk
 * \param[in] 	pEncrypted_chunk 	Encrypted chunk to decrypt
 * \param[out] 	pPlaintext_chunk 	Plaintext chunk
 * \return \ref stse_ReturnCode_t : STSAFE_OK on success ; error code otherwise
 * \details 	\include{doc} stse_aes_ecb_encrypt.dox
 */
stse_ReturnCode_t stsafea_aes_ecb_decrypt_process(
		stse_Handler_t * pSTSE,
		PLAT_UI16 chunk_length,
		PLAT_UI8 * pEncrypted_chunk,
		PLAT_UI8 * pPlaintext_chunk);

/**
 * \brief 		Finish chunk decryption in AES ECB mode
 * \details 	This service finish chunk decryption in AES ECB mode using the key specified in start command
 * \param[in] 	pSTSE 				Pointer to STSAFE Handler
 * \param[in] 	chunk_length 		Length of the chunk
 * \param[in] 	pEncrypted_chunk 	Encrypted chunk to decrypt
 * \param[out] 	pPlaintext_chunk 	Plaintext chunk
 ...
 * \details 	\include{doc} stse_aes_ecb_encrypt.dox
 */
stse_ReturnCode_t stsafea_aes_ecb_decrypt_finish(
		stse_Handler_t * pSTSE,
		PLAT_UI16 chunk_length,
		PLAT_UI8 * pEncrypted_chunk,
		PLAT_UI8 * pPlaintext_chunk);
