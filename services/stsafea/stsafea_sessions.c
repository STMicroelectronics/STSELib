...
/* Open a host session by querying device host-key metadata and initializing
 * local session context (key type, MAC counter, and key pointers).
 */
...
{
  /* Helper allows caller to switch active host session context explicitly. */
  if (pSTSE == NULL)
  {
    return STSE_SERVICE_HANDLER_NOT_INITIALISED;
  }
...
{
  /* Composite helper that applies optional payload encryption around the
   * authenticated transfer primitive.
   */
  stse_ReturnCode_t ret;
  ...
  {
    /* Null checks protect strap operations and frame head bitfield updates. */
    return STSE_SERVICE_SESSION_ERROR;
  }
  ...
  /* Response strap reserves encrypted storage before transfer updates payload. */
