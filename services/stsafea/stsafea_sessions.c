...
stse_ReturnCode_t stsafea_open_host_session(stse_Handler_t *pSTSE, stse_session_t *pSession,
					    PLAT_UI8 *pHost_MAC_key, PLAT_UI8 *pHost_cypher_key)
{
	...
	if (pSTSE->device_type == STSAFE_A120) {
		...
		pSession->context.host.MAC_counter =
			ARRAY_4B_SWAP_TO_UI32(host_key_slot.cmac_sequence_counter);
	} else {
		...
		pSession->context.host.MAC_counter =
			ARRAY_3B_SWAP_TO_UI32(host_key_slot.cmac_sequence_counter);
	}
...
stse_ReturnCode_t stsafea_session_frame_encrypt(stse_session_t *pSession, stse_frame_t *pFrame,
						stse_frame_element_t *pEnc_payload_element)
{
	...
	/* - Verify parameters */
	if ((pSession == NULL) || (pFrame == NULL) || (pEnc_payload_element == NULL) ||
	    (pEnc_payload_element->length < (pFrame->length - (pFrame->first_element->length + 1)) +
						    (16 - (pFrame->length % 16)))) {
	...
	ret = stse_platform_aes_ecb_enc(
		initial_value, STSAFEA_HOST_AES_BLOCK_SIZE, pSession->context.host.pHost_cypher_key,
		(pSession->context.host.key_type == STSE_AES_128_KT) ? STSE_AES_128_KEY_SIZE
	...
	while (pElement != NULL) {
		memcpy(pEnc_payload_element->pData + i, pElement->pData, pElement->length);
	...
	*(pEnc_payload_element->pData + i++) = 0x80;
	...
	ret = stse_platform_aes_cbc_enc(
		pEnc_payload_element->pData, pEnc_payload_element->length, initial_value,
		pSession->context.host.pHost_cypher_key,
		(pSession->context.host.key_type == STSE_AES_128_KT) ? STSE_AES_128_KEY_SIZE
								     : STSE_AES_256_KEY_SIZE,
		pEnc_payload_element->pData,
		NULL /*pEncFrame->first_element->length*/); /* TODO : Check that NULL is
							       OK */
	if (ret != 0) {
...
static stse_ReturnCode_t stsafea_session_frame_decrypt(stse_session_t *pSession,
						       stse_frame_t *pFrame)
{
	stse_ReturnCode_t ret;
	PLAT_UI8 initial_value[STSAFEA_HOST_AES_BLOCK_SIZE];
	...
	/* - Decrypt pRsp_Frame */
	ret = stse_platform_aes_cbc_dec(
		decrypt_buffer, pFrame->length - pFrame->first_element->length, initial_value,
		pSession->context.host.pHost_cypher_key,
		(pSession->context.host.key_type == STSE_AES_128_KT) ? STSE_AES_128_KEY_SIZE
								     : STSE_AES_256_KEY_SIZE,
		decrypt_buffer, NULL);
	...
	i = 0;
	while (pElement != NULL) {
		memcpy(pElement->pData, decrypt_buffer + i, pElement->length);
		i += pElement->length;
		pElement = pElement->next;
	}
							     ...
							     PLAT_UI8 *pMAC)
{
	...
	/*- create C-MAC Frame : [0x00] [CMD HEADER] [CMD PAYLOAD LENGTH] [CMD
	 * PAYLOAD] */
	stse_frame_allocate(c_mac_frame);
	stse_frame_element_allocate_push(&c_mac_frame, eMACType, 1, &mac_type);
	stse_frame_element_allocate_push(&c_mac_frame, eCMD_HEADER,
					 pCmd_frame->first_element->length,
					 pCmd_frame->first_element->pData);
	stse_frame_element_allocate_push(&c_mac_frame, eCmdPayloadLength, STSAFEA_CMD_RSP_LEN_SIZE,
	...
	while (pElement != NULL) {
		for (i = 0; i < pElement->length; i++) {
			if (aes_block_idx == STSAFEA_HOST_AES_BLOCK_SIZE) {
				stse_platform_aes_cmac_append(aes_cmac_block,
							    ...
							    PLAT_UI8 *pMAC)
{
	...
	if (*(pCmd_frame->first_element->pData) & STSAFEA_PROT_RSP_Msk) {
		...
		PLAT_UI16 rsp_payload_length =
			(pRsp_frame->length - (pRsp_frame->first_element->length));

		/*- Initialize AES C-MAC computation */
		stse_platform_aes_cmac_init(pSession->context.host.pHost_MAC_key,
					    (pSession->context.host.key_type == STSE_AES_128_KT)
						    ? STSE_AES_128_KEY_SIZE
						    : STSE_AES_256_KEY_SIZE,
					    STSAFEA_MAC_SIZE);
		...
		/*- Create r_mac_frame head :[MAC TYPE] [CMD HEADER] [CMD PAYLOAD LENGTH]
		 * [CMD PAYLOAD] ... */
		stse_frame_element_allocate_push(&r_mac_frame, eMACType, 1, &mac_type);

		stse_frame_element_allocate_push(&r_mac_frame, eCMD_header,
						 pCmd_frame->first_element->length,
						 pCmd_frame->first_element->pData);

		stse_frame_element_allocate_push(&r_mac_frame, eCMD_Length,
		...
		/*- Create r_mac_frame head : ... [RSP HEADER] [RSP PAYLOAD LENGTH] [RSP
		 * PAYLOAD] */
		stse_frame_element_allocate_push(&r_mac_frame, eRSP_header,
						 pRsp_frame->first_element->length,
						 pRsp_frame->first_element->pData);

		stse_frame_element_allocate_push(&r_mac_frame, eRsp_Length,
						 STSAFEA_CMD_RSP_LEN_SIZE,
...
stse_ReturnCode_t stsafea_session_encrypted_transfer(
	stse_session_t *pSession, stse_frame_t *pCmdFrame, stse_frame_t *pRspFrame,
	PLAT_UI8 cmd_encryption_flag, PLAT_UI8 rsp_encryption_flag,
	stse_cmd_access_conditions_t cmd_ac_info, PLAT_UI16 processing_time)
{
	stse_ReturnCode_t ret;
	...
	if (cmd_encryption_flag == 1) {
		PLAT_UI16 encrypted_payload_length =
			(pCmdFrame->length - pCmdFrame->first_element->length) + 1;
	...
	stse_frame_element_allocate(eEncrypted_cmd_payload, encrypted_cmd_payload_size,
				    encrypted_cmd_payload);
	...
	stse_frame_element_allocate(eEncrypted_rsp_payload, encrypted_rsp_payload_size,
				    encrypted_rsp_payload);
	...
	ret = stsafea_session_authenticated_transfer(pSession, pCmdFrame, pRspFrame, cmd_ac_info,
						     processing_time);
							 ...
							 PLAT_UI16 processing_time)
{
	...
	*(pCmdFrame->first_element->pData) |= ((1 << 7) | (1 << 6));
	...
	case STSE_HOST_SESSION:
		ret = stsafea_frame_raw_transfer(pSession->context.host.pSTSE, pCmdFrame, pRspFrame,
						 processing_time);
		if (ret == STSE_OK) {
			pSession->context.host.MAC_counter++;
		}
