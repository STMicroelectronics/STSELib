						    ...
						    PLAT_UI8 *pTotal_partition_count)
{
	...
	stse_frame_element_allocate_push(&RspFrame, eTotal_partition_count, 1,
					 pTotal_partition_count);
...
stse_ReturnCode_t
stsafea_get_data_partitions_configuration(stse_Handler_t *pSTSE, PLAT_UI8 total_partitions_count,
					  ...
					  PLAT_UI16 record_table_length)
{
	...
	if (ret == STSE_OK) {
		...
		for (partition_idx = 0; partition_idx < total_partitions_count; partition_idx++) {
			...
			pRecord_table->read_ac_cr =
				(stse_ac_change_right_t)STSAFEA_ZIR_AC_READ_CR_GET(raw_data[i]);
			pRecord_table->read_ac =
				(stse_zone_ac_t)STSAFEA_ZIR_AC_READ_GET(raw_data[i]);
			pRecord_table->update_ac_cr =
				(stse_ac_change_right_t)STSAFEA_ZIR_AC_UPDATE_CR_GET(raw_data[i]);
			pRecord_table->update_ac =
				(stse_zone_ac_t)STSAFEA_ZIR_AC_UPDATE_GET(raw_data[i++]);
...
stse_ReturnCode_t stsafea_decrement_counter_zone(stse_Handler_t *pSTSE, PLAT_UI8 zone_index,
						 stsafea_decrement_option_t option,
						 PLAT_UI32 amount, PLAT_UI16 offset,
						 PLAT_UI8 *pData, PLAT_UI8 data_length,
						 ...
						 stse_cmd_protection_t protection)
{
	...
	stse_frame_element_allocate_push(&CmdFrame, eOption, STSAFEA_ZONE_ACCESS_OPTION_SIZE,
					 (PLAT_UI8 *)&option);
	stse_frame_element_allocate_push(&CmdFrame, eZoneIndex, STSAFEA_ZONE_INDEX_SIZE,
					 &zone_index);
	stse_frame_element_allocate_push(&CmdFrame, eOffset, STSAFEA_ZONE_OFFSET_SIZE,
					 (PLAT_UI8 *)&offset);
	stse_frame_element_allocate_push(&CmdFrame, eAmount, STSAFEA_INC_DEC_AMOUT_SIZE,
					 (PLAT_UI8 *)&amount);
	...
	stse_frame_element_allocate_push(&RspFrame, eNewCounterVal, STSAFEA_COUNTER_VALUE_SIZE,
...
stse_ReturnCode_t stsafea_read_counter_zone(stse_Handler_t *pSTSE, PLAT_UI32 zone_index,
					    stsafea_read_option_t option, PLAT_UI16 offset,
					    ...
					    stse_cmd_protection_t protection)
{
	...
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEA_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eCounterVal, STSAFEA_COUNTER_VALUE_SIZE,
					 (PLAT_UI8 *)pCounter_value);
	stse_frame_element_allocate_push(&RspFrame, eAssociatedData, Associated_data_length,
					 pAssociated_data);
...
stse_ReturnCode_t stsafea_read_data_zone(stse_Handler_t *pSTSE, PLAT_UI32 zone_index,
					 stsafea_read_option_t option, PLAT_UI16 offset,
					 PLAT_UI8 *pReadBuffer, PLAT_UI16 read_length,
					 stse_cmd_protection_t protection)
{
	(void)protection;
	volatile stse_ReturnCode_t ret = STSE_SERVICE_INVALID_PARAMETER;
...
stse_ReturnCode_t stsafea_update_data_zone(stse_Handler_t *pSTSE, PLAT_UI32 zone_index,
					   stsafea_update_option_t option, PLAT_UI16 offset,
					   PLAT_UI8 *pData, PLAT_UI32 data_length,
					   stse_cmd_protection_t protection)
{
