...
/*! \defgroup stsafea_symmetric_key_slots STSAFE-A Symmetric key slots
...
typedef enum stsafea_symmetric_key_slot_t {
	...
	STSAFEA_EAP_PERIPHERAL_KEY_SLOTS =
		0x80,                 /*!< Enhanced Authentication Protocol peripheral key slots */
	STSAFEA_EAP_COMPANION_SLOT,   /*!< Enhanced Authentication Protocol companion
					 key slots */
	STSAFEA_EAP_KEY_CONFIRMATION, /*!< Enhanced Authentication Protocol key
					 confirmation */
...
typedef enum stsafea_symmetric_key_t {
	...
	STSAFEA_SYMMETRIC_KEY_TYPE_GENERIC_SECRET  /*!< Variable-length keys (from 16
						      to 32 bytes) */
...
typedef enum stsafea_symmetric_key_information_length_t {
	STSAFEA_KEY_INFORMATION_LENGTH_ECB =
		STSAFEA_SYMMETRIC_KEY_INFORMATION_BASE_LENGTH, /*!< AES-ECB mode */
...
typedef struct stsafe_generic_key_information_t {
	PLAT_UI16 info_length; /*!< stsafe_generic_key_information_t length excluding
				  this field */
	...
	union {
		struct {
			PLAT_UI8 auth_tag_length; /*!< Length of the authentication tag (0, 4, 8
						     or 16 bytes) */
			PLAT_UI8
			counter_offset_in_nonce: 7;   /*!< When counter present, it defines the
							 offset in the nonce where the counter
							 will be substituted (from 0 to 9)*/
			PLAT_UI8 counter_presence: 1; /*!< Counter presence indicator */
		} CCM;
		struct {
			PLAT_UI8
			min_MAC_length; /*!< Byte length of the MAC to verify (optional) */
		} CMAC;
		struct {
			PLAT_UI8 auth_tag_length; /*!< Length of the authentication tag (4, 8, 12,
						     13, 14, 15 or 16 bytes) */
		} GCM;
		struct {
			PLAT_UI8 filler: 7;                    /*!< Filler (Must be all 0b)*/
			PLAT_UI8 allow_derived_key_to_host: 1; /*!< Allow to return some keys
								  derived from the current key in
								  the response */
			PLAT_UI8
			generic_secret_key_length; /*!< Byte length of the key (only
						      present if key type ==
						      STSAFEA_SYMMETRIC_KEY_TYPE_GENERIC_SECRET
						    */
		} HKDF;
		struct {
			PLAT_UI8
 ...
 * \details 	This service format and send query symmetric key slot
 * provisioning control fields
 ...
 * \details 	This service format and send put symmetric key slot provisioning
 * control fields \param[in] 	pSTSE 				Pointer to STSE
 * Handler
 * \param[in] 	slot_number			Slot number of the slot to query
 * \param[in] 	pCtrl_fields		Structure for the provisioning control
 * fields of the target slot \return \ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
 ...
 * \details 	This service format and send query symmetric key table
 * informations \param[in] 	pSTSE
 * Pointer to STSE Handler \param[in] 	symmetric_key_slot_count	Expected
 * table slot count
 * \param[out] 	symmetric_key_table_info	Output key slot info list of
 * length equal to symmetric_key_slot_count \return \ref STSE_OK on success ;
 ...
 * \param[in] 	pSTSE 				Pointer to STSE Handler
 * \param[in] 	key_type 				Type of the key used in
 * ECDHE \param[in] 	host_ecdhe_public_key	Key used in ECDHE \return \ref
 * STSE_OK on success ; \ref stse_ReturnCode_t error code otherwise
 */
...
/**
 * \brief 		Authenticate establish symmetric key in stsafe symmetric
 * key table \details 	This service format and send establish symmetric key
 * command with an authentication signature \param[in] 	pSTSE
 * Pointer to STSE Handler \param[in] 	key_type
 * Type of the key used in ECDHE \param[in] 	host_ecdhe_public_key
 * Key used in ECDHE
 * \param[in] 	hash_algo
 * Signature hash algorithm ID \param[in] 	signature_public_key_slot_number
 * Generic public key slot number to be used for the authentication \param[in]
 * signature_key_type					Authentication signature
 * key type \param[in] 	pSignature Authentication signature \return \ref STSE_OK
 * on success ; \ref stse_ReturnCode_t error code otherwise
 */
...
/**
 * \brief 		Confirm symmetric key establishment in stsafe symmetric
 * key table \details 	This service format and send confirm symmetric key
 * command \param[in] 	pSTSE 				Pointer to STSE Handler
 * \param[in] 	pMac_confirmation_key 	Key used to MAC the confirmation command
 * payload \param[in] 	key_count				Number of key to
 * populate \param[in]	pKey_information_list	Key information list used to
 * populate the symmetric key table \return \ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
stse_ReturnCode_t
...
/**
 * \brief 		Write symmetric key envelope in stsafe symmetric key
 * table \details 	This service format and send write symmetric key command
 * \param[in] 	pSTSE 			 			Pointer to STSE
 * Handler
 * \param[in] 	pSymmetric_key_envelope 		Envelope containing key
 * materials to write in STSAFE \param[in] 	symmetric_key_envelope_length
 * Envelope length in bytes \return \ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
 ...
 * symmetric key slot \return \ref STSE_OK on success ; \ref stse_ReturnCode_t
 ...
 * \param[in] 	key_type 			\ref stse_aes_key_type_t key
 * wrap/unwrap key type \return \ref STSE_OK on success ; \ref stse_ReturnCode_t
 * error code otherwise
 */
 ...
 * \details 	This service erase content of specified key slot in symmetric
 * key table
 * \param[in] 	pSTSE 							Pointer
 * to STSE Handler \param[in] 	symmetric_key_slot_number Key's slot number in
