/**
  ******************************************************************************
  * @file    stsafea_symmetric_key_slots.h
  * @author  CS Application Team
  * @brief   STSAFE-A services for symmetric key slots management (header)
  ******************************************************************************
  * @attention
  *
...
{
  PLAT_UI16 info_length;                                    /*!< stsafe_generic_key_information_t
  ...
  union
  {
    struct
    {
      ...
      PLAT_UI8 counter_offset_in_nonce : 7; /*!< When counter present, it defines the
    ...
    struct
    {
      PLAT_UI8 allow_derived_key_to_host : 1; /*!< Allow to return some keys derived
                                                   from the current key in the response */
      PLAT_UI8 filler : 7;                    /*!< Filler (Must be all 0b)*/
      PLAT_UI8 generic_secret_key_length;     /*!< Byte length of the key (only present
                                                   if key type ==
...
/**
  * \brief     Query symmetric key table information
  * \details   This service format and send query symmetric key table information
  * \param[in]   pSTSE           Pointer to STSE Handler
