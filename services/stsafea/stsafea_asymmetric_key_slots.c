							...
							PLAT_UI8 *pPrivate_key_slot_count)
{
	...
	stse_frame_element_allocate_push(&RspFrame, ePrivate_key_slot_count, 1,
					 pPrivate_key_slot_count);
				...
				stsafea_private_key_slot_information_t *private_key_table_info)
{
	...
	PLAT_UI8 subject_tag = STSAFEA_SUBJECT_TAG_PRIVATE_KEY_TABLE;
	PLAT_UI8 rsp_header;
	PLAT_UI16 raw_table_length =
		private_key_slot_count * sizeof(stsafea_private_key_slot_information_t);
	...
	stse_frame_element_allocate_push(&RspFrame, eGlobal_usage_limit, 2,
					 (PLAT_UI8 *)pGlobal_usage_limit);
	...
	for (i = 0; i < slot_count; i++) {
		memset(&private_key_table_info[i], 0,
		       sizeof(stsafea_private_key_slot_information_t));
		memcpy(&private_key_table_info[i], current_record,
		...
		if (private_key_table_info[i].presence_flag == 1) {
			if (ARRAY_2B_SWAP_TO_UI16(current_record) >
			    STSE_ECC_CURVE_ID_VALUE_MAX_SIZE) {
				return STSE_UNEXPECTED_ERROR;
			}
			memcpy(&private_key_table_info[i].curve_id, current_record,
...
stse_ReturnCode_t stsafea_sign_for_generic_public_key_slot(stse_Handler_t *pSTSE,
							   stse_ecc_key_type_t private_key_type,
							   PLAT_UI8 *pPrivate_key,
							   stse_hash_algorithm_t hash_algo,
							   PLAT_UI16 payload_length,
							   PLAT_UI8 *pPayload, PLAT_UI8 *pSignature)
{
	(void)pSTSE;
	stse_ReturnCode_t ret;
	...
	{
		/* - Hash the payload */
		ret = stse_platform_hash_compute(hash_algo, pPayload, payload_length, tbs_data,
						 &hash_length);
	...
	ret = stse_platform_ecc_sign(private_key_type, pPrivate_key, tbs_data,
#ifdef STSE_CONF_ECC_EDWARD_25519
				     (private_key_type == STSE_ECC_KT_ED25519) ? payload_length
									       : hash_length,
#else
				     hash_length,
#endif
