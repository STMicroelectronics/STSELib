 ...
 * \details 		This service format and send/receive the generate random
 * command/response
 * \param[in]		pSTSE					Pointer to
 * target SE handler
 * \param[in]	 	pPassword_buffer 		Pointer to password
 * buffer
 * \param[in]	 	password_length 		Password length in bytes
 * \param[out]	 	pVerification_status 	Pointer to verification status
 * \param[out]	 	pRemaining_tries 		Pointer to remaining
 * tries \return 			\ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
stse_ReturnCode_t stsafea_verify_password(stse_Handler_t *pSTSE, PLAT_UI8 *pPassword_buffer,
					  PLAT_UI8 password_length, PLAT_UI8 *pVerification_status,
 ...
 * command/response
 * \param[in]		pSTSE					Pointer to
 * target SE handler \return 			\ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
