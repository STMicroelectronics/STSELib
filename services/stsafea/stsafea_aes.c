...
stse_ReturnCode_t stsafea_aes_ecb_encrypt(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
					  ...
					  PLAT_UI8 *pEncrypted_message)
{
	...
	stse_frame_element_allocate_push(&CmdFrame, ePlaintext_message, message_length,
					 pPlaintext_message);

	/* - Prepare RSP Frame : [HEADER] [ENCRYPTED MESSAGE]  */
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEA_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eEncrypted_message, message_length,
					 pEncrypted_message);
...
stse_ReturnCode_t stsafea_aes_ecb_decrypt(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
					  ...
					  PLAT_UI8 *pPlaintext_message)
{
	...
	/* - Prepare CMD Frame : [HEADER] [CMD DISTINGUISHER] [SLOT] [ENCRYPTED
	 * MESSAGE]  */
	stse_frame_allocate(CmdFrame);
	stse_frame_element_allocate_push(&CmdFrame, eCmd_header, STSAFEA_HEADER_SIZE, &cmd_header);
	stse_frame_element_allocate_push(&CmdFrame, eSub_command_distinguisher, 1,
					 &sub_command_distinguisher);
	stse_frame_element_allocate_push(&CmdFrame, eSlot_number, 1, &slot_number);
	stse_frame_element_allocate_push(&CmdFrame, eEncrypted_message, message_length,
					 pEncrypted_message);

	/* - Prepare RSP Frame : [HEADER] [PLAIN TEXT MESSAGE]  */
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEA_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, ePlaintext_message, message_length,
					 pPlaintext_message);
...
stse_ReturnCode_t stsafea_aes_ccm_encrypt(
	stse_Handler_t *pSTSE, PLAT_UI8 slot_number, PLAT_UI8 authentication_tag_length,
	PLAT_UI8 *pNonce, PLAT_UI16 associated_data_length, PLAT_UI8 *pAssociated_data,
	PLAT_UI16 message_length, PLAT_UI8 *pPlaintext_message, PLAT_UI8 *pEncrypted_message,
	PLAT_UI8 *pEncrypted_authentication_tag, PLAT_UI8 *pCounter_presence, PLAT_UI32 *pCounter)
{
	stse_ReturnCode_t ret;
	PLAT_UI8 cmd_header = STSAFEA_CMD_ENCRYPT;
	PLAT_UI8 sub_command_distinguisher = 0x02;
	...
	if ((pNonce == NULL) || (pAssociated_data == NULL && associated_data_length != 0) ||
	...
	/* - Prepare CMD Frame : [HEADER] [CMD DISTINGUISHER] [SLOT] [ASSOCIATED DATA
	 * LENGHT] ...
	 *                       ... [ASSOCIATED DATA MESSAGE] [MESSAGE LENGHT]
	 * [MESSAGE] */
	stse_frame_allocate(CmdFrame);
	stse_frame_element_allocate_push(&CmdFrame, eCmd_header, STSAFEA_HEADER_SIZE, &cmd_header);
	stse_frame_element_allocate_push(&CmdFrame, eSub_command_distinguisher, 1,
	...
	stse_frame_element_allocate_push(&CmdFrame, eAssociated_data_length,
					 STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&associated_data_length);
	stse_frame_element_allocate_push(&CmdFrame, eAssociated_data, associated_data_length,
					 pAssociated_data);
	...
	/* - Prepare RSP Frame : [HEADER] [ENCRYPTED MESSAGE] [TAG LENGTH] [COUNTER
	 * PRES.] [COUNTER VAL] */
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEA_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eEncrypted_message, message_length,
	...
	stse_frame_element_allocate_push(&RspFrame, eCounter, STSAFEA_COUNTER_VALUE_SIZE,
					 (PLAT_UI8 *)pCounter);
...
stse_ReturnCode_t stsafea_aes_ccm_encrypt_start(
	stse_Handler_t *pSTSE, PLAT_UI8 slot_number, PLAT_UI16 Nonce_length, PLAT_UI8 *pNonce,
	PLAT_UI16 total_associated_data_length, PLAT_UI32 total_message_length,
	PLAT_UI16 associated_data_chunk_length, PLAT_UI8 *pAssociated_data_chunk,
	PLAT_UI16 message_chunk_length, PLAT_UI8 *pPlaintext_message_chunk,
	PLAT_UI8 *pEncrypted_message_chunk, PLAT_UI8 *pCounter_presence, PLAT_UI32 *pCounter)
{
	stse_ReturnCode_t ret;
	...
	stse_frame_allocate(CmdFrame);
	stse_frame_element_allocate_push(&CmdFrame, eCmd_header, STSAFEA_HEADER_SIZE, &cmd_header);
	stse_frame_element_allocate_push(&CmdFrame, eCmd_header_ext, STSAFEA_HEADER_SIZE,
					 &cmd_header_ext);
	stse_frame_element_allocate_push(&CmdFrame, eSlot_number, 1, &slot_number);
	stse_frame_element_allocate_push(&CmdFrame, eNonce_length, STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&Nonce_length);
	stse_frame_element_allocate_push(&CmdFrame, eNonce, Nonce_length, pNonce);
	stse_frame_element_allocate_push(&CmdFrame, eTotal_associated_data_length,
					 STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&total_associated_data_length);
	stse_frame_element_allocate_push(&CmdFrame, eTotal_message_length, 4,
					 (PLAT_UI8 *)&total_message_length);
	stse_frame_element_allocate_push(&CmdFrame, eAssociated_data_length,
					 STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&associated_data_chunk_length);
	stse_frame_element_allocate_push(&CmdFrame, eAssociated_data, associated_data_chunk_length,
					 pAssociated_data_chunk);
	stse_frame_element_allocate_push(&CmdFrame, eMessage_length, STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&message_chunk_length);
	stse_frame_element_allocate_push(&CmdFrame, ePlaintext_message, message_chunk_length,
					 pPlaintext_message_chunk);

	/* - Prepare RSP Frame */
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEA_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, eEncrypted_message, message_chunk_length,
...
stse_ReturnCode_t stsafea_aes_ccm_encrypt_process(stse_Handler_t *pSTSE,
...
{
	return stsafea_aes_gcm_encrypt_process(pSTSE, associated_data_chunk_length,
					       pAssociated_data_chunk, message_chunk_length,
					       pPlaintext_message_chunk, pEncrypted_message_chunk);
}

stse_ReturnCode_t stsafea_aes_ccm_encrypt_finish(
	stse_Handler_t *pSTSE, PLAT_UI8 authentication_tag_length,
	PLAT_UI16 associated_data_chunk_length, PLAT_UI8 *pAssociated_data_chunk,
	PLAT_UI16 message_chunk_length, PLAT_UI8 *pPlaintext_message_chunk,
	PLAT_UI8 *pEncrypted_message_chunk, PLAT_UI8 *pEncrypted_authentication_tag)
{
	return stsafea_aes_gcm_encrypt_finish(
		pSTSE, authentication_tag_length, associated_data_chunk_length,
		pAssociated_data_chunk, message_chunk_length, pPlaintext_message_chunk,
		pEncrypted_message_chunk, pEncrypted_authentication_tag);
}

stse_ReturnCode_t stsafea_aes_ccm_decrypt(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
					  ...
					  PLAT_UI8 *pPlaintext_message)
{
	...
	/* - Prepare CMD Frame : [HEADER] [CMD DISTINGUISHER] [SLOT] [NONCE]
	 * [ASSOCIATED DATA LENGHT] ...
	 *                       ... [ASSOCIATED DATA] [MESSAGE LENGHT] [ENCRYPTED
	 * MESSAGE] [TAG] */
	stse_frame_allocate(CmdFrame);
	stse_frame_element_allocate_push(&CmdFrame, eCmd_header, STSAFEA_HEADER_SIZE, &cmd_header);
	stse_frame_element_allocate_push(&CmdFrame, eSub_command_distinguisher, 1,
...
stse_ReturnCode_t stsafea_aes_ccm_decrypt_start(
	stse_Handler_t *pSTSE, PLAT_UI8 slot_number, PLAT_UI16 Nonce_length, PLAT_UI8 *pNonce,
	PLAT_UI16 total_associated_data_length, PLAT_UI16 total_ciphertext_length,
	PLAT_UI16 associated_data_chunk_length, PLAT_UI8 *pAssociated_data_chunk,
	...
	PLAT_UI8 *pPlaintext_message_chunk)
{
	...
	stse_frame_element_allocate_push(&CmdFrame, eTotal_associated_data_length,
					 STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&total_associated_data_length);
	stse_frame_element_allocate_push(&CmdFrame, eTotal_ciphertext_length, 4,
					 (PLAT_UI8 *)&total_ciphertext_length);
	stse_frame_element_allocate_push(&CmdFrame, eAssociated_data_length,
					 STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&associated_data_chunk_length);
	stse_frame_element_allocate_push(&CmdFrame, eAssociated_data, associated_data_chunk_length,
	...
	stse_frame_element_allocate_push(&CmdFrame, eMessage_length, STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&message_chunk_length);
	stse_frame_element_allocate_push(&CmdFrame, eEncrypted_message, message_chunk_length,
					 pEncrypted_message_chunk);

	/* - Prepare RSP Frame */
	stse_frame_allocate(RspFrame);
	stse_frame_element_allocate_push(&RspFrame, eRsp_header, STSAFEA_HEADER_SIZE, &rsp_header);
	stse_frame_element_allocate_push(&RspFrame, ePlaintext_message, message_chunk_length,
					 pPlaintext_message_chunk);
...
stse_ReturnCode_t stsafea_aes_ccm_decrypt_process(stse_Handler_t *pSTSE,
						  PLAT_UI16 associated_data_chunk_length,
						  PLAT_UI8 *pAssociated_data_chunk,
						  PLAT_UI16 message_chunk_length,
						  PLAT_UI8 *pEncrypted_message_chunk,
						  PLAT_UI8 *pPlaintext_message_chunk)
{
	return stsafea_aes_gcm_decrypt_process(pSTSE, associated_data_chunk_length,
					       pAssociated_data_chunk, message_chunk_length,
					       pEncrypted_message_chunk, pPlaintext_message_chunk);
}

stse_ReturnCode_t
stsafea_aes_ccm_decrypt_finish(stse_Handler_t *pSTSE, PLAT_UI8 authentication_tag_length,
			       PLAT_UI16 associated_data_chunk_length,
			       ...
			       PLAT_UI8 *pVerification_result, PLAT_UI8 *pPlaintext_message_chunk)
{
	return stsafea_aes_gcm_decrypt_finish(
		pSTSE, authentication_tag_length, associated_data_chunk_length,
		pAssociated_data_chunk, message_chunk_length, pEncrypted_message_chunk,
		pAuthentication_tag, pVerification_result, pPlaintext_message_chunk);
}

stse_ReturnCode_t stsafea_aes_gcm_encrypt(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
					  ...
					  PLAT_UI8 *pAuthentication_tag)
{
	...
	stse_frame_element_allocate_push(&CmdFrame, eSub_command_distinguisher, 1,
					 &sub_command_distinguisher);
	stse_frame_element_allocate_push(&CmdFrame, eSlot_number, 1, &slot_number);
	stse_frame_element_allocate_push(&CmdFrame, eIV_length, STSAFEA_GENERIC_LENGTH_SIZE,
					 (PLAT_UI8 *)&IV_length);
...
stse_ReturnCode_t stsafea_aes_gcm_encrypt_start(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
...
stse_ReturnCode_t stsafea_aes_gcm_encrypt_process(stse_Handler_t *pSTSE,
						  PLAT_UI16 associated_data_chunk_length,
						  PLAT_UI8 *pAssociated_data_chunk,
						  PLAT_UI16 message_chunk_length,
						  PLAT_UI8 *pPlaintext_message_chunk,
...
stse_ReturnCode_t
stsafea_aes_gcm_encrypt_finish(stse_Handler_t *pSTSE, PLAT_UI8 authentication_tag_length,
			       PLAT_UI16 associated_data_chunk_length,
			       PLAT_UI8 *pAssociated_data_chunk, PLAT_UI16 message_chunk_length,
			       PLAT_UI8 *pPlaintext_message_chunk,
			       PLAT_UI8 *pEncrypted_message_chunk, PLAT_UI8 *pAuthentication_tag)
...
stse_ReturnCode_t stsafea_aes_gcm_decrypt(
	stse_Handler_t *pSTSE, PLAT_UI8 slot_number, PLAT_UI8 authentication_tag_length,
	PLAT_UI16 IV_length, PLAT_UI8 *pIV, PLAT_UI16 associated_data_length,
	PLAT_UI8 *pAssociated_data, PLAT_UI16 message_length, PLAT_UI8 *pEncrypted_message,
	PLAT_UI8 *pAuthentication_tag, PLAT_UI8 *pVerification_result, PLAT_UI8 *pPlaintext_message)
{
	PLAT_UI8 cmd_header = STSAFEA_CMD_DECRYPT;
	PLAT_UI8 sub_command_distinguisher = 0x02;
	PLAT_UI8 rsp_header;
	...
	/* - Prepare CMD Frame : [HEADER] [CMD DISTINGUISHER] [SLOT] [IV] [ASSOCIATED
	 * DATA LENGHT] ...
	 *                       ... [ASSOCIATED DATA] [MESSAGE LENGHT] [ENCRYPTED
	 * MESSAGE] [TAG] */
	stse_frame_allocate(CmdFrame);
...
stse_ReturnCode_t stsafea_aes_gcm_decrypt_start(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
						PLAT_UI16 IV_length, PLAT_UI8 *pIV,
						PLAT_UI16 associated_data_chunk_length,
						PLAT_UI8 *pAssociated_data_chunk,
						PLAT_UI16 message_chunk_length,
...
stse_ReturnCode_t stsafea_aes_gcm_decrypt_process(stse_Handler_t *pSTSE,
						  PLAT_UI16 associated_data_chunk_length,
						  PLAT_UI8 *pAssociated_data_chunk,
						  PLAT_UI16 message_chunk_length,
						  PLAT_UI8 *pEncrypted_message_chunk,
...
stse_ReturnCode_t
stsafea_aes_gcm_decrypt_finish(stse_Handler_t *pSTSE, PLAT_UI8 authentication_tag_length,
			       PLAT_UI16 associated_data_chunk_length,
			       PLAT_UI8 *pAssociated_data_chunk, PLAT_UI16 message_chunk_length,
			       PLAT_UI8 *pEncrypted_message_chunk, PLAT_UI8 *pAuthentication_tag,
			       PLAT_UI8 *pVerification_result, PLAT_UI8 *pPlaintext_message_chunk)
