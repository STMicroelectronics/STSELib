...
/* This source centralizes STSAFE-A symmetric cipher service commands for
 * one-shot and streaming AES-ECB/CCM/GCM operations.
 */

#ifdef STSE_CONF_STSAFE_A_SUPPORT

/* AES-ECB one-shot encryption helper.
 * Command payload carries sub-command selector, key slot, and plaintext bytes.
 * Response payload returns encrypted bytes of identical length.
 */
...
/* AES-ECB one-shot decryption helper.
 * Input ciphertext length and output plaintext length are identical.
 * The function mirrors the encryption framing with DECRYPT command header.
 */
...
/* AES-CCM one-shot encryption helper.
 * Command includes nonce, associated data, and plaintext payload.
 * Response returns ciphertext, authentication tag, and optional counter value.
 */
...
/* AES-CCM streaming START phase.
 * Initializes device context with nonce and total lengths for upcoming chunks.
 * Returns first encrypted chunk and optional counter information.
 */
  ...
  PLAT_UI32 *pCounter)
{
  ...
  if ((pNonce == NULL) || (pAssociated_data_chunk == NULL && associated_data_chunk_length != 0)
      ...
      || (pEncrypted_message_chunk == NULL && message_chunk_length != 0) || (pEncrypted_message_chunk != NULL
                                                                             && message_chunk_length == 0))
  {
    return (STSE_SERVICE_INVALID_PARAMETER);
  }
...
/* AES-CCM streaming PROCESS phase.
 * Reuses GCM process framing helper because chunk wire layout is identical.
 */
...
/* AES-CCM streaming FINISH phase.
 * Final chunk can produce both ciphertext tail and authentication tag bytes.
 */
...
/* AES-CCM one-shot decryption helper.
 * Command includes nonce, AAD, ciphertext, and encrypted authentication tag.
 * Response exposes verification status plus plaintext output bytes.
 */
  ...
  PLAT_UI8 *pPlaintext_message)
{
  ...
  if ((pNonce == NULL) || (pAssociated_data == NULL && associated_data_length != 0) || (pAssociated_data != NULL
      ...
      || (pPlaintext_message != NULL && message_length == 0)
      || (pEncrypted_authentication_tag == NULL && authentication_tag_length != 0)
      || (pEncrypted_authentication_tag != NULL && authentication_tag_length == 0) || (pVerification_result == NULL))
  {
    return (STSE_SERVICE_INVALID_PARAMETER);
  }
...
/* AES-CCM streaming START phase for decryption.
 * Initializes decrypt context with nonce and total ciphertext length.
 * Returns first plaintext chunk if message data is present in start call.
 */
  ...
  PLAT_UI8 *pPlaintext_message_chunk)
{
  ...
  if ((pNonce == NULL) || (pAssociated_data_chunk == NULL && associated_data_chunk_length != 0)
      || (pAssociated_data_chunk != NULL && associated_data_chunk_length == 0) || (pEncrypted_message_chunk == NULL
          && message_chunk_length != 0) || (pEncrypted_message_chunk != NULL && message_chunk_length == 0)
      || (pPlaintext_message_chunk == NULL && message_chunk_length != 0) || (pPlaintext_message_chunk != NULL
                                                                             && message_chunk_length == 0))
  {
    return (STSE_SERVICE_INVALID_PARAMETER);
  }
...
/* AES-CCM streaming PROCESS phase for decryption.
 * Uses shared GCM process helper since chunk framing is common.
 */
...
/* AES-CCM streaming FINISH phase for decryption.
 * Sends final ciphertext bytes and tag, receives verification result.
 */
  ...
  PLAT_UI8 *pAuthentication_tag)
{
  ...
  if ((pIV == NULL || IV_length == 0) || (pAssociated_data == NULL && pPlaintext_message == NULL)
      || (pAssociated_data == NULL && associated_data_length != 0)
      || (pAssociated_data != NULL && associated_data_length == 0)
      || (pPlaintext_message == NULL && message_length != 0)
      || (pPlaintext_message != NULL && message_length == 0)
  ...
  {
    /* Parameter consistency checks avoid ambiguous length/pointer combinations. */
    return (STSE_SERVICE_INVALID_PARAMETER);
  }
...
/* AES-GCM streaming START phase for encryption.
 * Supplies IV and first AAD/message chunks to initialize device state.
 */
...
/* AES-GCM streaming PROCESS phase for encryption.
 * Feeds intermediate AAD/message chunks and returns ciphertext chunks.
 */
...
/* AES-GCM streaming FINISH phase for encryption.
 * Processes final chunks and outputs tail ciphertext plus authentication tag.
 */
...
/* AES-GCM one-shot decryption helper.
 * Command carries IV, AAD, ciphertext, and tag; response carries verification
 * result and plaintext bytes.
 */
...
/* AES-GCM streaming START phase for decryption.
 * Initializes decrypt stream with IV and first ciphertext/AAD chunks.
 */
...
/* AES-GCM streaming PROCESS phase for decryption.
 * Continues decrypt stream with intermediate AAD/ciphertext chunks.
 */
...
/* AES-GCM streaming FINISH phase for decryption.
 * Finalizes authentication with tag verification and returns verification flag.
 */
