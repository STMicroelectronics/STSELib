...
#define STSAFEA_ZIR_AC_READ_CR_GET(x)                                                              \
	(((x)&STSAFEA_ZIR_AC_READ_CR_Msk) >> STSAFEA_ZIR_AC_READ_CR_Pos)
#define STSAFEA_ZIR_AC_READ_GET(x) (((x)&STSAFEA_ZIR_AC_READ_Msk) >> STSAFEA_ZIR_AC_READ_Pos)
#define STSAFEA_ZIR_AC_UPDATE_CR_GET(x)                                                            \
	(((x)&STSAFEA_ZIR_AC_UPDATE_CR_Msk) >> STSAFEA_ZIR_AC_UPDATE_CR_Pos)
...
typedef struct stsafea_read_option_t {
	...
	stse_ac_change_right_t new_read_ac_change_right
		: STSE_1BIT_LEN; /*!< Access condition change right */
	stse_zone_ac_change_indicator_t change_ac_indicator
		: STSE_1BIT_LEN;         /*!< Access condition change indicator */
...
typedef struct stsafea_update_option_t {
	...
	stse_ac_change_right_t new_update_ac_change_right
		: STSE_1BIT_LEN; /*!< Access condition change right */
	stse_zone_ac_change_indicator_t change_ac_indicator
		: STSE_1BIT_LEN;         /*!< Access condition change indicator */
...
typedef struct stsafea_decrement_option_t {
	...
	stse_ac_change_right_t new_decrement_ac_change_right
		: STSE_1BIT_LEN; /*!< Access condition change right */
	stse_zone_ac_change_indicator_t change_ac_indicator
		: STSE_1BIT_LEN;         /*!< Access condition change indicator */
	PLAT_UI8 filler : STSE_3BIT_LEN; /*!< Must be 0b0 */
 ...
 * \details 		This service format and send/receive STSAFE-Axxx start
 * hash command/response \param[in]		pSTSE
 * Pointer to target SE handler \param[out]		total_partition_count
 * total partition count in bytes \return 			\ref STSE_OK on
 ...
 * \details 		This service format and send/receive STSAFE-Axxx start
 * hash command/response \param[in]		pSTSE
 * Pointer to target SE handler \param[in]		total_partitions_count
 * Total partition count in bytes \param[in,out]	pRecord_table
 * Pointer to applicative partition record table \param[in]
 * record_table_length		Applicative record table length \return
...
stse_ReturnCode_t stsafea_decrement_counter_zone(stse_Handler_t *pSTSE, PLAT_UI8 zone_index,
						 ...
						 PLAT_UI32 amount, PLAT_UI16 offset, PLAT_UI8 *data,
						 PLAT_UI8 data_length, PLAT_UI32 *new_counter_value,
						 stse_cmd_protection_t protection);

stse_ReturnCode_t stsafea_read_counter_zone(stse_Handler_t *pSTSE, PLAT_UI32 zone_index,
					    stsafea_read_option_t read_ac, PLAT_UI16 read_offset,
...
stse_ReturnCode_t stsafea_update_data_zone(stse_Handler_t *pSTSE, PLAT_UI32 zone_index,
					   ...
					   PLAT_UI8 *data, PLAT_UI32 data_length,
					   stse_cmd_protection_t protection);
