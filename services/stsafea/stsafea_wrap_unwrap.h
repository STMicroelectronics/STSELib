 ...
 * \details 	This service format and send STSAFEA wrap command/response to
 * target STSE
 * \param[in]	pSTSE					Pointer to target SE
 * handler \param[in]	wrap_key_slot			Wrap key slot \param[in]
 * pPayload				Pointer to the payload buffer to be
 * wrapped \param[in]	payload_size			size of the payload
 * buffer \param[out] 	pWrapped_Payload		Pointer to the wrapped
 * payload buffer \param[in]	wrapped_payload_size	size of the wrapped
 * payload buffer \return \ref STSE_OK on success ; \ref stse_ReturnCode_t error
 * code otherwise
 */
 ...
 * \details 	This service format and send STSAFEA un-wrap command/response to
 * target STSE
 * \param[in]	pSTSE					Pointer to target SE
 * handler \param[in]	wrap_key_slot			wrap key slot \param[in]
 * pWrapped_Payload		Pointer to the wrapped payload buffer to be
 * un-wrapped
 * \param[in]	wrapped_payload_size	Size of the wrapped payload buffer
 * \param[out]	pPayload				Pointer to the plain
 * text payload buffer \param[in]	payload_size Size of the payload buffer
