...
#define STSAFEA_KEK_HKDF_SALT                                                                      \
	{                                                                                          \
		0xE4, 0x5D, 0x4C, 0xFD, 0x20, 0x15, 0xDE, 0x9E, 0x0A, 0xA9, 0xA9, 0xF6, 0xA4,      \
			0x9F, 0x45, 0x10, 0x0F, 0x99, 0xD9, 0x63, 0x3F, 0xB1, 0xB2, 0xDD, 0xDB,    \
			0xEE, 0xE6, 0x75, 0xD5, 0x4D, 0xE2, 0x5B                                   \
	}
 ...
 * \details 	This service format and send query host key provisioning control
 * fields command \param[in] 	pSTSE 				Pointer to STSE
 * Handler \param[out] 	pCtrl_fields		Pointer to the provisioning
 * control fields \return \ref stse_ReturnCode_t : STSAFEA_OK on success ; error
 * code otherwise
 */
stse_ReturnCode_t stsafea_query_host_key_provisioning_ctrl_fields(
	stse_Handler_t *pSTSE, stsafea_host_key_provisioning_ctrl_fields_t *pCtrl_fields);

/**
 * \brief 		Put host key provisioning control fields
 * \details 	This service format and send put host key provisioning control
 * fields command \param[in] 	pSTSE 				Pointer to STSE
 * Handler \param[out] 	pCtrl_fields		Pointer to the provisioning
 * control fields \return \ref stse_ReturnCode_t : STSAFEA_OK on success ; error
 * code otherwise
 ...
 * \param[out] 	pHostKeySlot		Pointer to the structure to output key
 * informations \return \ref stse_ReturnCode_t : STSAFEA_OK on success ; error
 * code otherwise
 */
stse_ReturnCode_t stsafea_query_host_key(stse_Handler_t *pSTSE,
 ...
 * \details 	This service format and send query host key informations command
 * \param[in] 	pSTSE 			Pointer to STSE Handler
 * \param[out] 	pHostKeySlotV2		Pointer to the structure to output key
 * informations \return \ref stse_ReturnCode_t : STSAFEA_OK on success ; error
 * code otherwise
 */
stse_ReturnCode_t stsafea_query_host_key_v2(stse_Handler_t *pSTSE,
 ...
 */
stse_ReturnCode_t stsafea_put_attribute_host_key(stse_Handler_t *pSTSE,
 ...
 * \param[in] 	host_keys			Host key structure to be
 * provisioned \return \ref stse_ReturnCode_t : STSAFEA_OK on success ; error
 * code otherwise
 */
stse_ReturnCode_t stsafea_host_key_provisioning(stse_Handler_t *pSTSE,
 ...
 * \param[in] 	pSTSE 			Pointer to STSE Handler
 * \param[in] 	key_type			Host key type
 * \param[in] 	pHost_key_envelope	Envelope containing host key to be
 * provisioned \return \ref stse_ReturnCode_t : STSAFEA_OK on success ; error
 * code otherwise
 */
stse_ReturnCode_t stsafea_host_key_provisioning_wrapped(stse_Handler_t *pSTSE,
							stsafea_host_key_type_t key_type,
 ...
 * Handler \param[in]   host_ecdh_public_key_type	ECDHE host public key
 * type \param[in]   pPublic_key 				ECDHE host
 * public key \param[in] 	host_keys_type				Host key
 * type \return \ref stse_ReturnCode_t : STSAFEA_OK on success ; error code
 * otherwise
 */
stse_ReturnCode_t stsafea_establish_host_key(stse_Handler_t *pSTSE,
					     stse_ecc_key_type_t host_ecdh_public_key_type,
					     PLAT_UI8 *pPublic_key,
					     stsafea_host_key_type_t host_keys_type);

/**
 * \brief 		Authenticated establish host key using ECDH & HKDF
 * processes \details 	Provision host key using the authenticated establish
 * host key command
 * \param[in] 	pSTSE 						Pointer to STSE
 * Handler \param[in]   host_ecdh_public_key_type	ECDHE host public key
 * type \param[in]   pPublic_key 				ECDHE host
 * public key \param[in] 	host_keys_type				Host key
 * type \param[in] 	signature_public_key_slot	Generic public key slot
 * holding key used to verify signature \param[in] signature_public_key_type
 * Public key's type stored through generic public key slot \param[in]
 * signature_hash_algo			Hashing algorithm used for the signature
 * \param[in] 	pSignature					Pointer to
 * buffer containing signature \return \ref stse_ReturnCode_t : STSAFEA_OK on
 * success ; error code otherwise
 */
stse_ReturnCode_t stsafea_establish_host_key_authenticated(
	stse_Handler_t *pSTSE, stse_ecc_key_type_t host_ecdh_public_key_type, PLAT_UI8 *pPublic_key,
	stsafea_host_key_type_t host_keys_type, PLAT_UI8 signature_public_key_slot,
	stse_ecc_key_type_t signature_public_key_type, stse_hash_algorithm_t signature_hash_algo,
	PLAT_UI8 *pSignature);
