...
#define STSAFEA_SUBJECT_TAG_DATA_PARTITION_CONFIGURATION 0x12              \
  /*!< data partition configuration subject-tag */
...
#define STSAFEA_SUBJECT_TAG_COMMAND_AUTHORIZATION_CONFIG 0x24              \
  /*!< Command authorisation configuration subject-tag */
...
#define STSAFEA_SUBJECT_TAG_PATCH_SIGNATURE_VERIFICATION_KEY 0x43          \
  /*!< Patch signature verification key subject-tag */
#define STSAFEA_SUBJECT_TAG_PATCH_SIGNATURE_CONFIGURATION_FLAGS 0x44       \
  /*!< Patch signature configuration flags subject-tag */
#define STSAFEA_SUBJECT_TAG_PATCH_UPDATE_COMMAND_COUNTER 0x45              \
  /*!< Patch update command counter subject-tag */
#define STSAFEA_SUBJECT_TAG_PATCH_PARENT_KEY_USAGE_LIMIT 0x46              \
  /*!< Patch Parent key usage limit subject-tag */
...
#define STSAFEA_SUBJECT_TAG_HOST_KEY_SLOT_PROVISIONING_CONTROL_FIELDS 0x4A \
  /*!< Host key slot provitioning control fields subject-tag */
#define STSAFEA_SUBJECT_TAG_SYMMETRIC_KEY_SLOT_PROVISIONING_CONTROL 0x4B   \
  /*!< Symmetric key slot provitioning control subject-tag */
#define STSAFEA_SUBJECT_TAG_GENERIC_PUBLIC_KEY_SLOT 0x4C                   /*!< Generic public key slot subject-tag */
#define STSAFEA_SUBJECT_TAG_GENERIC_PUBLIC_KEY_CONFIGURATION_FLAGS 0x4D    \
  /*!< Generic public key configuration flags subject-tag */
...
PLAT_UI8 i2c_paramers_lock : STSE_1BIT_LEN;        /*!< Lock I2C configuration defined by other
                                                     parameters (0: none, 1: lock) */
PLAT_UI8 low_power_mode : STSE_2BIT_LEN;           /*!< Low power mode (0: none, 2: standby) */
PLAT_UI8 idle_bus_time_to_standby : STSE_5BIT_LEN; /*!< For standby, period of time without any
                                                     command processing and with I2C bus idle until
                                                     the chip enters standby mode. Coded in 50ms
                                                     steps, starting from 50ms as floor value
                                                     (0: 50ms, 1: 100ms, ..., 31: 1600ms) */
