...
stse_ReturnCode_t stsafea_frame_transmit(stse_Handler_t *pSTSE, stse_frame_t *pFrame)
{
	...
	while ((retry_count != 0) && (ret == STSE_PLATFORM_BUS_ACK_ERROR)) {
		...
		ret = pSTSE->io.BusSendStart(pSTSE->io.busID, pSTSE->io.Devaddr, pSTSE->io.BusSpeed,
					     pFrame->length);
		...
		if (ret == STSE_OK) {
			...
			while (pCurrent_element != pFrame->last_element) {
				ret = pSTSE->io.BusSendContinue(
					pSTSE->io.busID, pSTSE->io.Devaddr, pSTSE->io.BusSpeed,
					pCurrent_element->pData, pCurrent_element->length);
				if (ret != STSE_OK) {
					break;
				}
...
stse_ReturnCode_t stsafea_frame_receive(stse_Handler_t *pSTSE, stse_frame_t *pFrame)
{
	...
	while ((retry_count != 0) && (ret == STSE_PLATFORM_BUS_ACK_ERROR)) {
		/* - Receive frame length from target STSAFE */
		ret = pSTSE->io.BusRecvStart(pSTSE->io.busID, pSTSE->io.Devaddr, pSTSE->io.BusSpeed,
	...
	pSTSE->io.BusRecvContinue(pSTSE->io.busID, pSTSE->io.Devaddr, pSTSE->io.BusSpeed, NULL,
				  STSE_RSP_FRAME_HEADER_SIZE);

	/* - Get STSAFE Response Length */
	ret = pSTSE->io.BusRecvStop(pSTSE->io.busID, pSTSE->io.Devaddr, pSTSE->io.BusSpeed,
				    length_value, STSE_FRAME_LENGTH_SIZE);
	if (ret != STSE_OK) {
		return (ret);
	}
	...
	stse_frame_element_allocate_push(pFrame, eFiller, filler_size, filler);
	...
	while ((retry_count != 0) && (ret == STSE_PLATFORM_BUS_ACK_ERROR)) {
		/* - Receive frame length from target STSAFE */
		ret = pSTSE->io.BusRecvStart(pSTSE->io.busID, pSTSE->io.Devaddr, pSTSE->io.BusSpeed,
					     STSE_FRAME_LENGTH_SIZE + received_length +
						     STSE_FRAME_CRC_SIZE);
	...
	ret = pSTSE->io.BusRecvContinue(pSTSE->io.busID, pSTSE->io.Devaddr, pSTSE->io.BusSpeed,
	...
	while (pCurrent_element != pFrame->last_element) {
		...
		ret = pSTSE->io.BusRecvContinue(pSTSE->io.busID, pSTSE->io.Devaddr,
						pSTSE->io.BusSpeed, pCurrent_element->pData,
...
stse_ReturnCode_t stsafea_frame_raw_transfer(stse_Handler_t *pSTSE, stse_frame_t *pCmdFrame,
					     stse_frame_t *pRspFrame, PLAT_UI16 inter_frame_delay)
{
					 ...
					 stse_frame_t *pRspFrame)
{
	...
	if (pCmdFrame->first_element != NULL && pCmdFrame->first_element->pData != NULL) {
		if (pCmdFrame->first_element->length == STSAFEA_HEADER_SIZE) {
			if (pCmdFrame->first_element->pData[0] == STSAFEA_EXTENDED_COMMAND_PREFIX) {
				...
				if (pCmdFrame->first_element->next->length == STSAFEA_HEADER_SIZE &&
				    pCmdFrame->first_element->next->pData != NULL) {
					cmd_header_extended =
						pCmdFrame->first_element->next->pData[0];
					inter_frame_delay =
						stsafea_extended_cmd_timings[pSTSE->device_type]
									    [cmd_header_extended];
					stsafea_perso_info_get_ext_cmd_AC(pSTSE->pPerso_info,
									  cmd_header_extended,
									  &cmd_ac_info);
					stsafea_perso_info_get_ext_cmd_encrypt_flag(
						pSTSE->pPerso_info, cmd_header_extended,
						&cmd_encryption_flag);
					stsafea_perso_info_get_ext_rsp_encrypt_flag(
						pSTSE->pPerso_info, cmd_header_extended,
						&rsp_encryption_flag);
				}
			} else {
				cmd_header = pCmdFrame->first_element->pData[0];
				inter_frame_delay = stsafea_extended_cmd_timings[pSTSE->device_type]
										[cmd_header];
				stsafea_perso_info_get_cmd_AC(pSTSE->pPerso_info, cmd_header,
							      &cmd_ac_info);
				stsafea_perso_info_get_cmd_encrypt_flag(
					pSTSE->pPerso_info, cmd_header, &cmd_encryption_flag);
				stsafea_perso_info_get_rsp_encrypt_flag(
					pSTSE->pPerso_info, cmd_header, &rsp_encryption_flag);
			}
			ret = STSE_OK;
		}
	}
	...
	if (cmd_encryption_flag || rsp_encryption_flag) {
		ret = stsafea_session_encrypted_transfer(
			pSTSE->pActive_host_session, pCmdFrame, pRspFrame, cmd_encryption_flag,
			rsp_encryption_flag, cmd_ac_info, inter_frame_delay);
	} else if (cmd_ac_info != STSE_CMD_AC_FREE) {
		ret = stsafea_session_authenticated_transfer(pSTSE->pActive_host_session, pCmdFrame,
							     pRspFrame, cmd_ac_info,
							     inter_frame_delay);
	} else
#endif
	{
		ret = stsafea_frame_raw_transfer(pSTSE, pCmdFrame, pRspFrame, inter_frame_delay);
	}
