...
/*!
 *  \defgroup stsafea_asymmetric_key_crypto STSAFE-A Asymmetric key slots
 * management \ingroup stsafea_services \brief STSAFEA Asymmetric key
 * cryptography services \details The STSELib services layer provide set of
 * functions that format all commands supported \n by the targeted secure
 * element and reports response to higher layers API/Application
 *  @{
 */
...
typedef struct stsafea_private_key_operation_mode_t {
	/* 1st byte */
	PLAT_UI8 ephemeral_private_key_gen_method
		: STSE_1BIT_LEN;               /*!< ephemeral_private_key_gen_method */
	...
	PLAT_UI8 sig_gen_over_internal_external_data
		: STSE_1BIT_LEN;                  /*!< sig_gen_over_internal_external_data*/
 ...
 * \details 	This service format and send the get private key slot count
 * command
 * \param[in] 	pSTSE 						Pointer to STSE
 * Handler \param[out] 	pPrivate_key_slot_count 	Target STSAFE-A total
 * Private key slot count \return 		\ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise
 */
stse_ReturnCode_t stsafea_query_private_key_slots_count(stse_Handler_t *pSTSE,
							PLAT_UI8 *pPrivate_key_slot_count);

stse_ReturnCode_t
stsafea_query_private_key_table(stse_Handler_t *pSTSE, PLAT_UI8 private_key_slot_count,
				PLAT_UI8 *pChange_right, PLAT_UI16 *pGlobal_usage_limit,
				stsafea_private_key_slot_information_t *private_key_table_info);

stse_ReturnCode_t stsafea_generate_ecc_key_pair(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
 ...
 * \param[in] 	pSTSE 						Pointer to STSE
 * Handler
 * \param[in] 	key_type 					Curve type for
 * the ECDHE key pair \param[out] 	pPublic_key generated ECDHE public key
 * \return 		\ref STSE_OK on success ; \ref stse_ReturnCode_t error
 * code otherwise
 */
stse_ReturnCode_t stsafea_generate_ECDHE_key_pair(stse_Handler_t *pSTSE,
						  stse_ecc_key_type_t key_type,
						  PLAT_UI8 *pPublic_key);

stse_ReturnCode_t stsafea_sign_for_generic_public_key_slot(
	stse_Handler_t *pSTSE, stse_ecc_key_type_t private_key_type, PLAT_UI8 *pPrivate_key,
	stse_hash_algorithm_t hash_algo, PLAT_UI16 payload_length, PLAT_UI8 *pPayload,
	PLAT_UI8 *pSignature);
