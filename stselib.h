 ...
 *  \details  	The STSE Certificate parser is the entry point for the upper
 * system application layer. It provides high level certificate parsing
 * functions to the application layer.
 *
 *  \defgroup 	stse_api STSE API layer
 *  \brief		Common API set for STMicroelectronics brand protection
 * secure elements \details  	The STSE API layer is the entry point for the
 * upper system application layer. It provides high level functions allowing
 * seamless implementation of advanced authentication , SE cryptographic
 * processing  and device life cycle monitoring at applicative level  .
 *
 *  \defgroup 	stse_services STSE Services layer
 *  \brief		Secure Element specific services Layer
 *  \details  	The Services layer provide set of functions that format all
 * commands supported \n by a specific secure element from STMicroelectronics
 * Brand Protection portfolio
 *
 *  \defgroup 	stse_core STSE Core layer
 *  \brief		Frame protocol , hardware abstraction and cryptographic
 * abstraction layer \details  	The Core layer provide SE frame management
 * services and abstraction to specific HW and cryptographic platforms required
 * by the STSE library to operate on specific HOST
 */
#ifdef __cplusplus
extern "C" {
#endif
