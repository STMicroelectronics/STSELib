						   ...
						   PLAT_UI16 *pCertificate_size)
{
	...
	*pCertificate_size = (((uint16_t)certificate_size_ui8[0]) << 8) +
			     ((uint16_t)certificate_size_ui8[1]) + 4U;
					      ...
					      PLAT_UI16 certificate_size, PLAT_UI8 *pCertificate)
{
	...
	ret = stse_data_storage_read_data_zone(pSTSE,                         /* STSE handler */
					       certificate_zone,              /* Certificate zone */
...
stse_ReturnCode_t stse_device_authenticate(stse_Handler_t *pSTSE,
					   ...
					   PLAT_UI8 certificate_zone, PLAT_UI8 priv_key_slot_number)
{
	...
	ret = stse_data_storage_read_data_zone(
		pSTSE, certificate_zone, STSE_CERTIFICATE_SIZE_OFFSET_BYTES, /* 2 bytes offset */
		certificate_size_ui8,         /* Returned certificate size */
		STSE_CERTIFICATE_SIZE_LENGTH, /* Certificate size length 2 bytes */
		0,                            /* No maximum chunck size (No chunck at all) */
		STSE_NO_PROT);                /* No protection */
	...
	certificate_size = (((uint16_t)certificate_size_ui8[0]) << 8) +
			   ((uint16_t)certificate_size_ui8[1]) + 4U;
	...
	ret = stse_certificate_parse_chain((uint8_t *)pRoot_CA_certificate, certificate,
					   certificate_size, &leaf_certificate);
	if (ret != STSE_OK) {
		return (ret);
	}
	...
	ret = stse_ecc_generate_signature(pSTSE,                /* STSE handler */
					  priv_key_slot_number, /* Slot number */
					  key_type, challenge,  /* Random number to sign */
	...
	ret = stse_certificate_verify_signature(
		&leaf_certificate, challenge, challenge_size, signature, (signature_size >> 1),
