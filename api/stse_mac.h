 ...
 *  \details  	The Message Authentication Code (MAC) API set provides high
 * level functions to MAC generation/verification.
 *  \{
 */
 ...
 */
stse_ReturnCode_t stse_cmac_hmac_compute(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
 ...
 * \param[in] 	pSTSE 				Pointer to STSE Handler
 * \param[in] 	slot_number 			Key slot in symmetric key table
 * to be used
 * \param[in] 	pMac 					Buffer containing the
 * MAC
 * \param[in]	mac_length				MAC length
 * (CMAC:2,4,8,16 / HMAC:16-32) \param[in]	pMessage
 * Plaintext message
 * \param[in]	message_length			Plaintext message length
 * \param[out]	verification_result		Verification result flag
 * \return \ref STSE_OK on success ; \ref stse_ReturnCode_t error code otherwise
 */
stse_ReturnCode_t stse_cmac_hmac_verify(stse_Handler_t *pSTSE, PLAT_UI8 slot_number, PLAT_UI8 *pMac,
					PLAT_UI8 mac_length, PLAT_UI8 *pMessage,
					PLAT_UI8 message_length, PLAT_UI8 *verification_result);
 ...
 * key table to be used \param[in]	IV_length
 * IV buffer length in bytes \param[in]	pIV
 * IV buffer \param[in]	associated_data_length		Length of the associated
 * data \param[in]	pAssociated_data			Buffer
 * containing associated data
 * \param[in] 	authentication_tag_length 	Expected length for the
 * authentication tag
 ...
 */
stse_ReturnCode_t
stse_aes_gmac_compute(stse_Handler_t *pSTSE, PLAT_UI8 slot_number, PLAT_UI16 IV_length,
 ...
 * \return \ref STSE_OK on success ; \ref stse_ReturnCode_t error code otherwise
 */
stse_ReturnCode_t stse_aes_gmac_verify(stse_Handler_t *pSTSE, PLAT_UI8 slot_number,
