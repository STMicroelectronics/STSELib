...
#define IS_PUB_KEY_COMPRESSED(point_representation_id) ((point_representation_id == PUB_KEY_COMPRESSED_EVEN) \
...
/**
  ...
  * \param[in]   eddsa_variant       Flag indicating a pre-hashed EdDSA (1)
  *            message or a pure EdDSA (0) plaintext message. Used only in case
  *            of Edwards25519 public key
  * \param[out]  pSignature_validity   Signature validity flag (1 = valid signature, invalid otherwise)
  * \return     \ref STSE_OK on success ; \ref stse_ReturnCode_t error code otherwise
  *
  * \note The expected signature format is the concatenation of R and S values
  *       (R|S as defined in ASN.1 DER encoding).
  *       The public key format must match the expected ECC representation
  *       (typically concatenation of X and Y coordinates for uncompressed, or X
  *       with a prefix for compressed)
  *
...
/**
  ...
  *
  * \note The public key format must match the expected ECC representation
  *       (typically concatenation of X and Y coordinates for uncompressed, or X
  *       with a prefix for compressed).
  */
