  ...
  PLAT_UI8 *pAuthentication_tag)
{
  ...
  return stsafea_aes_gmac_compute(pSTSE, slot_number, IV_length, pIV, associated_data_length, pAssociated_data,
                                  authentication_tag_length, pAuthentication_tag);
#else
  return STSE_API_INCOMPATIBLE_DEVICE_TYPE;
#endif /* STSE_CONF_STSAFE_A_SUPPORT */
}
  ...
  PLAT_UI8 *pVerification_result)
{
  ...
  return stsafea_aes_gmac_verify(pSTSE, slot_number, IV_length, pIV, associated_data_length, pAssociated_data,
                                 authentication_tag_length, pAuthentication_tag, pVerification_result);
#else
  return STSE_API_INCOMPATIBLE_DEVICE_TYPE;
#endif /* STSE_CONF_STSAFE_A_SUPPORT */
}
