...
{
  (void)pSTSE;
  /* This helper signs host-side data using provisioned private key material;
   * secure element transport is not used in this code path. */
  ...
  /* Hash algorithm table provides output length used for stack buffer sizing. */
  ...
  if (private_key_type != STSE_ECC_KT_ED25519)
  {
    ...
    /* Non-Ed25519 signatures are computed over digest, not raw message bytes. */
    /* - Hash the payload */
  ...
  /* For Ed25519, signing API consumes payload directly per algorithm requirements. */
