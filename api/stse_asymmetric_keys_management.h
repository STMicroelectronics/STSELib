...
/** \defgroup   stse_api_asymmetric_key_mngt  STSE Asymmetric key management
  ...
  *              It provides high level functions to the application layer for
  *              symmetric asymmetric key management on target
  *              STMicroelectronics Secure element device.
  *  @{
  */
...
/**
  * \brief     Get ECC key table information
  * \details   This API query STSE and return ecc key information list for all key slots in the table
  * \param[in]   pSTSE             Pointer to STSE Handler
  ...
  * \param[out]  private_key_table_info    Key slot information table
  * \return \ref STSE_OK on success ; \ref stse_ReturnCode_t error code otherwise
...
/**
  * \brief     Get ECC key slot information
  * \details   This API query STSE and return ecc key information for a given key slot
  * \param[in]   pSTSE             Pointer to STSE Handler
  * \param[in]   private_key_slot_number   Slot number to query
  * \param[out]  pGlobal_usage_limit     Private key table information global usage limit
  * \param[out]  private_key_slot_info     Key slot information
  * \return \ref STSE_OK on success ; \ref stse_ReturnCode_t error code otherwise
...
/**
  ...
  * \details   This API generated signature for authenticating an off-chip
  *            entity with its public key in a generic public key slot
  * \param[in]   pSTSE         Pointer to STSE Handler
  ...
  *          pPrivate_key must reference the concatenated key pair buffer.
  */
