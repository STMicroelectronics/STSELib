 ...
 * functions to the application layer for symmetric asymmetric key management on
 ...
 * \details 	This API query STSE and return the number of slots in the
 * private key table
 * \param[in] 	pSTSE 						Pointer to STSE
 * Handler \param[out] 	pPrivate_key_slot_count 	ECC key slots count
 ...
 * \details 	This API query STSE and return ecc key informations list for all
 * key slots in the table \param[in] 	pSTSE
 * Pointer to STSE Handler \param[in] 	private_key_slot_count
 * Expected slot count in the table (length of private_key_table_info)
 * \param[out] 	pChange_right 				Private key table
 * information change right
 * \param[out] 	pGlobal_usage_limit 		Private key table information
 * global usage limit \param[out] 	private_key_table_info 		Key slot
 * informations table \return \ref STSE_OK on success ; \ref stse_ReturnCode_t
 * error code otherwise \details 	\include{doc}
 * stse_get_ecc_key_table_info.dox
 */
stse_ReturnCode_t
 ...
 * \details 	This API query STSE and return ecc key informations for a given
 * key slot
 * \param[in] 	pSTSE 						Pointer to STSE
 * Handler \param[in] 	private_key_slot_number 	Slot number to query
 * \param[out] 	pChange_right 				Private key table
 * information change right
 * \param[out] 	pGlobal_usage_limit 		Private key table information
 * global usage limit \param[out] 	private_key_slot_info 		Key slot
 * informations \return \ref STSE_OK on success ; \ref stse_ReturnCode_t error
 * code otherwise \details 	\include{doc} stse_get_ecc_key_slot_info.dox
 */
stse_ReturnCode_t
 ...
 * \details 	This API request STSE to generate an ECC key pair in the private
 * key table \param[in] 	pSTSE 			Pointer to STSE Handler
 * \param[in] 	slot_number 	Private key table slot used to generate the key
 * pair \param[in] 	key_type 		Key pair type \param[in]
 * usage_limit 	Usage limit of the key pair \param[out] 	pPublic_key
 * Public key of the generated key pair \return \ref STSE_OK on success ; \ref
 * stse_ReturnCode_t error code otherwise \details 	\include{doc}
 * stse_generate_ecc_key_pair.dox
 */
