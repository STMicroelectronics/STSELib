  ...
  PLAT_UI8 num_keys)
{
  ...
  {
    /* Each entry requests one plaintext derived key returned in the response buffer.
     * Key material destination and length are encoded per map item. */
  ...
  PLAT_UI16 key_length)
{
  ...
  /* The IKM is transported in command data and consumed atomically by the device;
   * this wrapper does not persist the source key material in device key slots. */
