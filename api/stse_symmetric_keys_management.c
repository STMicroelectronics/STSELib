  ...
  stse_ecc_key_type_t ecc_key_type)
{
  ...
  ret = stse_platform_hmac_sha256_compute(pHkdf_salt,
                                          ...
                                          shared_secret,
                                          stse_ecc_info_table[ecc_key_type].shared_secret_size,
                                          pHkdf_info,
                                          2 * pub_key_size,
  ...
  PLAT_UI8 *private_key)
{
  ...
  memcpy(pTBS,
         (PLAT_UI8 *)&stse_ecc_info_table[ecc_key_type].curve_id,
...
#endif /* host/symmetric key provisioning wrapped authenticated */
  ...
  PLAT_UI8 *signature_private_key)
{
  ...
  host_key_envelope[STSE_HOST_KEY_ENVELOPE_FRONT_PADDING_LENGTH + STSE_HOST_KEY_ENVELOPE_KEY_TYPE_LENGTH +
                    host_keys_length] = 0x80;
  ...
  PLAT_UI8 *host_cipher_key)
{
  ...
  PLAT_UI16 host_mac_key_length = (host_secure_channel_keys_type == STSAFEA_AES_256_HOST_KEY)
                                  ? STSAFEA_HOST_AES_256_MAC_KEY_SIZE
                                  : STSAFEA_HOST_AES_128_MAC_KEY_SIZE;
  PLAT_UI16 host_cipher_key_length = (host_secure_channel_keys_type == STSAFEA_AES_256_HOST_KEY)
  ...
  PLAT_UI8 *host_cipher_key)
{
  ...
  ret = stsafea_query_generic_public_key_slot_info(pSTSE,
                                                   tbs_public_key_slot,
                                                   &tbs_presence_flag,
                                                   &tbs_configuration_flags,
                                                   &tbs_key_type);
  if (ret != STSE_OK)
  {
    return ret;
  ...
  PLAT_UI8 *private_key)
{
  ...
  /* - Prepare asymmetric keys and shared secret buffers */
  PLAT_UI8 host_ecdhe_public_key[stse_ecc_info_table[ecc_key_type].public_key_size];
