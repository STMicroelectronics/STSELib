					 ...
					 PLAT_UI16 Partitioning_table_length)
{
	...
	return stsafea_get_data_partitions_configuration(
		pSTSE, total_partition_count, pPartitioning_table, Partitioning_table_length);
}
						   ...
						   stse_cmd_protection_t protection)
{
	...
	/* - Set read option with all zero for both members of the union as they act
	 * similarly */
	...
	do {
		...
		case STSAFE_L010:
			ret = stsafel_read_data_zone(
				pSTSE, zone, read_option.stsafel, chunck_offset,
				pBuffer + (chunck_offset)-offset, chunck_length, protection);
			break;
#endif
		...
		case STSAFE_A200:
			ret = stsafea_read_data_zone(
				pSTSE, zone, read_option.stsafea, chunck_offset,
				pBuffer + (chunck_offset)-offset, chunck_length, protection);
			break;
#endif
		default:
						     ...
						     stse_cmd_protection_t protection)
{
	...
	/* - Set update option with all zero for both members of the union as they act
	 * similarly */
...
#ifdef STSE_CONF_STSAFE_L_SUPPORT
	case STSAFE_L010:
		ret = stsafel_update_data_zone(pSTSE, zone, update_option.stsafel, offset, pBuffer,
					       length, protection);
		break;
#endif
#ifdef STSE_CONF_STSAFE_A_SUPPORT
	...
	case STSAFE_A120:
	case STSAFE_A200:
		ret = stsafea_update_data_zone(pSTSE, zone, update_option.stsafea, offset, pBuffer,
					       length, protection);
		break;
#endif
	default:
							   ...
							   stse_cmd_protection_t protection)
{
	...
	/* - Set decrement option with all zero for both members of the union as they
	 * act similarly */
...
#ifdef STSE_CONF_STSAFE_L_SUPPORT
	case STSAFE_L010:
		ret = stsafel_decrement_counter_zone(pSTSE, zone, decrement_option.stsafel, amount,
						     offset, pBuffer, length, new_counter_value,
						     protection);
		break;
#endif
#ifdef STSE_CONF_STSAFE_A_SUPPORT
	...
	case STSAFE_A120:
	case STSAFE_A200:
		ret = stsafea_decrement_counter_zone(pSTSE, zone, decrement_option.stsafea, amount,
						     offset, pBuffer, length, new_counter_value,
						     protection);
		break;
#endif
						      ...
						      stse_cmd_protection_t protection)
{
	...
	do {
...
#ifdef STSE_CONF_STSAFE_L_SUPPORT
		case STSAFE_L010:
			ret = stsafel_read_counter_zone(pSTSE, zone, read_option.stsafel,
							chunck_offset,
							pBuffer + (chunck_offset)-offset,
							chunck_length, pCounter_value, protection);
			break;
#endif
#ifdef STSE_CONF_STSAFE_A_SUPPORT
		case STSAFE_A100:
		...
		case STSAFE_A120:
		case STSAFE_A200:
			ret = stsafea_read_counter_zone(pSTSE, zone, read_option.stsafea,
							chunck_offset,
							pBuffer + (chunck_offset)-offset,
							chunck_length, pCounter_value, protection);
			break;
	...
	stse_ac_change_right_t ac_change_right, stse_cmd_protection_t protection)
{
	...
	ret = stsafea_read_data_zone(pSTSE, zone, options, 0x00, NULL, 0x00, protection);
	...
	PLAT_UI16 length, stse_zone_update_atomicity_t atomicity, stse_cmd_protection_t protection)
{
	...
	/*- Update zone */
	ret = stsafea_update_data_zone(pSTSE, zone, options, offset, pBuffer, length, protection);
	...
	stse_cmd_protection_t protection)
{
	...
	return stsafea_decrement_counter_zone(pSTSE, zone, options, amount, offset, pBuffer, length,
					      new_counter_value, protection);
}
