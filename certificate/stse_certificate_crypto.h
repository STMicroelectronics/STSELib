...
/**
 * \brief  Verify the signature of a digest with a public key extracted from the
 * certificate
 * \param[in]  cert 			pointer to the parsed x509 certificate
 * containing the public key to be used for the signature verification
 * \param[in]  digest 			the digest of the message to be verified
 * \param[in]  digestSize 		size in bytes of the digest of the
 * message to be verified \param[in]  signatureR 		the R value of
 * the
 * signature to be verified \param[in]  signatureRsize 	the size in bytes of
 * signatureR
 * \param[in]  signatureS 		the S value of the signature to be
 * verified \param[in]  signatureSsize 	the size in bytes of signatureS \return
...
stse_ReturnCode_t stse_certificate_verify_signature(const stse_certificate_t *cert,
						    const PLAT_UI8 *digest, PLAT_I32 digestSize,
 ...
 * certificate of the certificated supposedly issued by parent \return \ref
 ...
 * \param[in]  cert 			pointer to the parsed x509 certificate
 * \return \ref stse_ecc_key_type_t : certificate public key type ; 0xFF if key
 * type is invalid
 */
 ...
 * \param[in]  cert 			pointer to the parsed x509 certificate
 * \return \ref stse_hash_algorithm_t : certificate signature hash algorithm ;
 * 0xFF if hash algo is invalid
 */
