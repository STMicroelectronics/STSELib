...
PLAT_I32 stse_certificate_identify_ASN1_TLV(const PLAT_UI8 *asn1, PLAT_I32 *parsed, PLAT_I32 *size,
                                            const PLAT_UI8 **value)
{
  /* Core DER primitive decoder used by all higher-level certificate parsers. */
  /* TLV format has the TAG in the first byte*/
  /* This helper returns both TLV tag and payload boundaries in a single pass. */
  ...
  *parsed = 0;
  ...
  /* Second byte tells us the size if it's < 128.
   * Otherwise if bit7 is set, it tells how many bytes encode the size.
   */
  if (asn1[1] >> 7 == 1U) /* if bit7 is set, then size is expressed over multiple byte*/
  {
    /* Long-form length: lower 7 bits tell number of subsequent length octets. */
  ...
  /* We write both the pointer to value (which follows the size)
   * and the number of bytes scanned.
   */
  *parsed = 2 + (PLAT_I32)count;
  /* value points to first payload octet, immediately after tag+length bytes. */
  *value = asn1 + *parsed;
  /* And we return the tag */
  return (tag);
}

void stse_certificate_parse_integer(const PLAT_UI8 *integer, const PLAT_UI8 **outp, PLAT_I32 *outSize,
                                    const PLAT_UI8 **next_thing)
{
  /* Extract INTEGER payload pointer/size and return pointer to following object. */
  ...
  if (tag == TAG_INTEGER)
  {
    ...
    /* ASN.1 INTEGER can include a leading 0x00 to force positive sign. */
  ...
  *next_thing = integer + parsed + size;
  /* next_thing allows caller to chain-decoder through enclosing sequence. */
}

void stse_certificate_parse_ECDSA_signature(const PLAT_UI8 *signature, stse_certificate_t *stse_certificate,
                                            const PLAT_UI8 **next_thing)
{
  /* Decode signature BIT STRING to internal R/S fields based on algorithm type. */
  ...
  if (tag == TAG_BITSTRING && next[0] == 0x00U)
  {
    ...
    /* Ed25519 signatures are fixed 64-byte blobs split into R and S halves. */
  ...
  *next_thing = signature + wholesize + parsed;
  /* Caller resumes parsing right after outer BIT STRING TLV. */
}

PLAT_I32 stse_certificate_get_small_integer(const PLAT_UI8 *value, PLAT_I32 size)
{
  /* Convert short positive ASN.1 INTEGER into host PLAT_I32 when representable. */
  PLAT_UI32 retval = 0;
  PLAT_I32 i = 0;
  ...
  {
    /* Reject values that do not fit positive 32-bit signed domain. */
    return (-1);
  }
...
void stse_certificate_parse_signature_algorithm(const PLAT_UI8 *SA, PLAT_I32 *singatureAlgorithm,
                                                const PLAT_UI8 **next_thing)
{
  /* Identify signature algorithm by matching DER OID against supported table. */
#define N_OF_IDENTIFIABLE_SIGNATURE_ALGORITHMS 6
  ...
  /* OID table normalizes parser output to internal algorithm enum values. */
  ...
  *next_thing = SA + total_size;
  /* total_size already includes SEQUENCE header bytes consumed by parser. */
#undef N_OF_IDENTIFIABLE_SIGNATURE_ALGORITHMS
}

void stse_certificate_parse_X509_version(const PLAT_UI8 *x509VersionField, stse_certificate_t *stse_certificate,
                                         const PLAT_UI8 **next_thing)
{
  /* Parse optional X.509 explicit version field from TBSCertificate. */
  ...
  if (tag == TAG_INTEGER)
  {
    /* X509 version is a small INTEGER typically encoded as 0/1/2 for v1/v2/v3. */
    stse_certificate->x509Version = stse_certificate_get_small_integer(next, size);
  }
  *next_thing = x509VersionField + parsed + size;
}

void stse_certificate_parse_ECC_public_key(const PLAT_UI8 *EccPK, stse_certificate_t *stse_certificate,
                                           const PLAT_UI8 **next_thing)
{
  /* Decode SubjectPublicKeyInfo and normalize key bytes/curve information. */
  PLAT_I32 i, total_size = 0, size = 0, parsed, tag;
  const PLAT_UI8 *next;
#define N_OF_IDENTIFIABLE_ECS 10
  ...
  /* Parser maps known curve OIDs to internal enum used by downstream crypto checks. */
  ...
  if (tag == TAG_SEQUENCE)
  {
    ...
    if (tag == TAG_SEQUENCE)
    {
      ...
      if (tag == TAG_OBJECT_IDENTIFIER)
      {
        /* First OID usually identifies id-ecPublicKey / EdDSA algorithm family. */
        const PLAT_UI8 ecdsa_with_sha_oid[] = {42U, 134U, 72U, 206U, 61U, 2U, 1U};
        /* Some certificates include algorithm OID before named-curve OID. */
        if ((memcmp(next, ecdsa_with_sha_oid, sizeof(ecdsa_with_sha_oid)) == 0)
            && (size == (PLAT_I32)sizeof(ecdsa_with_sha_oid)))
        {
          next += 7;
          tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
        }
        if (tag == TAG_OBJECT_IDENTIFIER)
        {
          /* Second OID (or first after optional skip) identifies the concrete curve. */
          ...
          {
            /* BIT STRING starts with "unused bits" byte, then point/key bytes. */
            if ((next[0] == 0x00U) && (stse_certificate->EllipticCurve != EC_Ed25519))
            {
              ...
              switch (next[1])
              {
                ...
                case 0x02U:
                case 0x03U:
                  /* Compressed point: sign byte + X only; Y to be reconstructed later. */
                  stse_certificate->PubKey.fsize = (size - 2);
                  stse_certificate->PubKey.pX = &next[2];
                  stse_certificate->pPubKey_point_representation_id = &next[1];
                  break;
                default:
                  /* Fallback: unknown representation, keep raw bytes for caller handling. */
            ...
            {
              /* Ed25519 and non-standard forms are stored as raw key blob from next[1]. */
              stse_certificate->PubKey.fsize = (size - 1);
              stse_certificate->PubKey.pX = &next[1];
              stse_certificate->pPubKey_point_representation_id = 0;
            }
  ...
  *next_thing = EccPK + total_size;
  /* next_thing points to object immediately after SubjectPublicKeyInfo. */
#undef N_OF_IDENTIFIABLE_ECS
}

PLAT_I32 stse_certificate_identify_attribute(const PLAT_UI8 *oid, PLAT_I32 size)
{
  /* Map Distinguished Name attribute OID to compact internal enum value. */
#define N_OF_IDENTIFIABLE_ATTRIBUTES 9
  ...
  {
    /* Compare both encoded length and OID bytes to avoid prefix collisions. */
...
void stse_certificate_parse_relative_distinguished_name(const PLAT_UI8 *p, const PLAT_UI8 **nextRDN,
                                                        const PLAT_UI8 **attribute)
{
  /* Move across one RDN entry and expose contained attribute TLV start pointer. */
  PLAT_I32 size = 0, parsed;
  (void)stse_certificate_identify_ASN1_TLV(p, &parsed, &size, attribute);
  /* nextRDN points right after the current attribute set payload. */
  *nextRDN = *attribute + size;
}

PLAT_I32 stse_certificate_case_identify_extension(const PLAT_UI8 *oid, PLAT_I32 size)
{
  /* Identify extension type from OID to drive extension-specific parsing path. */
#define N_OF_IDENTIFIABLE_EXTENSIONS 3
  ...
  {
    /* Exact length+content comparison prevents confusion between extension arcs. */
...
{
  /* Lightweight case-insensitive ASCII compare for certificate textual fields. */
  PLAT_I32 i; /* For loop*/
  ...
  for (i = 0; i < size; i++)
  {
    /* ASCII-only normalization by xor-difference keeps implementation lightweight. */
    d = p1[i] ^ p2[i];
    if (d != 0U)
    {
      if (d == 32U
...
void stse_certificate_parse_attribute(const PLAT_UI8 *p, stse_cert_attribute_t *attribute_st,
                                      const PLAT_UI8 **next_thing)
{
  /* Decode one AttributeTypeAndValue sequence and keep raw value pointer/format. */
  ...
  if (tag == TAG_SEQUENCE)
  {
    /* Attribute sequence usually encodes OID followed by string-like value node. */
    tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
    if (tag == TAG_OBJECT_IDENTIFIER)
    {
      ...
      {
        /* Keep raw string pointer/length to avoid extra copies during parse stage. */
        next += size;
        tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
  ...
  *next_thing = p + total_size;
  /* total_size includes full AttributeTypeAndValue wrapper length. */
}

PLAT_I32 stse_certificate_count_attributes(const PLAT_UI8 *p)
{
  /* Count number of AVAs in Name sequence without allocating or copying values. */
  ...
  {
    /* Scans all RDNs */
    /* The function counts AVAs without decoding individual OID/value payloads. */
    while (next < p + total_size)
    {
      ...
      if (tag == TAG_SET)
      {
        /* Each sequence inside the RDN set contributes one attribute entry. */
        while (next < EoSet)
        {
          tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
          if (tag == TAG_SEQUENCE)
...
void stse_certificate_parse_validity(const PLAT_UI8 *p, stse_cert_validity_t *notBefore_st,
                                     stse_cert_validity_t *notAfter_st, const PLAT_UI8 **next_thing)
{
  /* Parse notBefore/notAfter timestamps from validity sequence into numeric fields. */
  PLAT_I32 i, total_size = 0, size = 0, parsed, tag;
  const PLAT_UI8 *next = p;
  ...
  if (tag == TAG_SEQUENCE)
  {
    /* Validity sequence is expected to contain exactly notBefore and notAfter nodes. */
    for (i = 0; i < 2; i++)
    {
      if (i == 0)
      {
      ...
      if (tag == TAG_UTC_TIME || tag == TAG_GENERALIZED_TIME)
      {
        ...
        if (tag == TAG_UTC_TIME)
        {
          /* UTCTime uses 2-digit year with RFC5280 pivot at 1950/2050. */
        ...
        {
          /* GeneralizedTime carries 4-digit year. */
        ...
        /* Remaining fields are parsed as decimal ASCII pairs: DDhhmmss. */
        ...
        /* Advance over any remaining suffix (for example trailing 'Z'). */
        next += size;
      }
    }
  }
  if (next_thing != NULL)
  {
    /* Preserve null-check so callers can parse validity in place when no continuation is needed. */
    *next_thing = p + total_size;
  }
}
...
{
  /* Compare two parsed date structures and return ordering in {-1,0,1}. */
  PLAT_I32 test = 0;
  /* Lexicographic compare from most to least significant date field. */
  ...
  /* Return contract uses {-1,0,1} only to simplify caller branch logic. */
...
{
  /* Parse selected X.509 extensions and pack results into compact flag bitfield. */
  ...
  if (tag == TAG_OBJECT_IDENTIFIER)
  {
    /* extFlags bitfield encodes extension presence, criticality and key-usage bits. */
    ext_st->type = stse_certificate_case_identify_extension(next, size);
    /* Unknown OIDs leave ext_st->type unmatched and are ignored by default branch. */
    next += size;

    switch (ext_st->type)
    {
      case EXTENSION_BC:
        /* BasicConstraints parsing extracts CA flag and optional pathLen. */
        /* ext_st->value keeps raw encoded extension body for optional external inspection. */
        ext_st->critical = 0;
        *extFlags |= 1U; /* BC is present */
        /* BC flags layout: bit0 present, bit1 critical, bit2 isCA, bits[7:4] pathLen. */
        /* RFC5280 allows critical boolean to be absent; default is non-critical. */
        tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
        /* Either we get the boolean or the octet string */
        if (tag == TAG_BOOLEAN)
        {
          /* BOOLEAN TRUE marks extension as critical for certificate processing. */
          if (size == 1 && next[0] != 0U)
          {
            ext_st->critical = 1;
        ...
        {
          /* Extension OCTET STRING wraps an inner DER payload. */
          ext_st->value = next;
          ext_st->valueSize = size;

          /* Recurse once into wrapped payload to decode semantic fields. */

          tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &seqSize, &next);
          /* It must start with a sequence */
          if (tag == TAG_SEQUENCE)
          {
            /* Not CA and No Path */
            /* Empty sequence means cA=false and pathLen absent. */
            if (seqSize > 0) /* There is a CA or PATH */
            {
              ...
              /* pathLen is optional and appears only when CA flag permits constraints. */
              /* Is there more ? */
              if (next < ext_st->value + ext_st->valueSize)   /* Yes, then it's the path */
              {
                /* pathLen is stored on 4 bits in extFlags for compact representation. */
                tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
                if (tag == TAG_INTEGER)
                {
                  /* Keep only low 4 bits to match compact extFlags contract. */
                  *extFlags |= 1U << 3; /* Path is present */
                  *extFlags |=
                    ((PLAT_UI32)stse_certificate_get_small_integer(next, size) & 15U)
                    << 4; /* 4-bit encoding for pathLen */
                }
              }
            }
          }
        ...
        break;
      case EXTENSION_KU:
        /* KeyUsage bitstring is unpacked into dedicated extFlags positions. */
        /* KU parsing stores both presence/critical bits and key-usage mask bits. */
        ext_st->critical = 0;
        *extFlags |= 1U << 8; /* KU is present */
        /* KU flags occupy higher extFlags bits to avoid overlap with BC encoding. */
        /* Same optional-critical handling as BasicConstraints. */
        tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
        /* Either we get the boolean or the octet string */
        if (tag == TAG_BOOLEAN)
        {
          /* Critical KU means relying party must understand this extension. */
          if (size == 1 && next[0] != 0U)
          {
            ext_st->critical = 1;
        ...
        if (tag == TAG_OCTETSTRING)
        {
          ...
          /* Wrapped BIT STRING holds key usage bits as defined by RFC5280. */

          tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &seqSize, &next);
          /* It must start with a sequence */
          if (tag == TAG_BITSTRING)
          {
            /* next[0] is number of unused bits in final key-usage octet. */
            /* This reads the 8 bits (or less) and put it in the right place */
            /* Bits are re-packed into extFlags starting from bit16. */
            ...
            {
              /* Handle ninth key-usage bit when BIT STRING spans two content bytes. */
