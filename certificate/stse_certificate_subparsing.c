					    ...
					    const PLAT_UI8 **value)
{
	...
	*parsed = 0;
	...
	/*Second byte tells us about the size if it's < 128. Otherwise if bit7 is set,
	 * it tell us how many bytes encode the size */
	if (asn1[1] >> 7 == 1U) /* if bit7 is set, then size is expressed over multiple byte*/
	{
		count = (PLAT_UI32)asn1[1] &
			127U; /* Read the number of bytes expressing the size */
		/* Read the size */
		*size = 0;
		for (i = 0; i < count; i++) {
			*size <<= 8U;
	...
	/* We effectivly write as output both the pointer to value (which follows the
	 * size) and the number of bytes we've scanned */
...
void stse_certificate_parse_integer(const PLAT_UI8 *integer, const PLAT_UI8 **outp,
				    PLAT_I32 *outSize, const PLAT_UI8 **next_thing)
{
...
void stse_certificate_parse_ECDSA_signature(const PLAT_UI8 *signature,
					    ...
					    const PLAT_UI8 **next_thing)
{
	...
	/* We expect a BITSTRING with the first byte (indicating the number of bits to
	 * exclude form the LSB) to be zero */
	...
	if (tag == TAG_BITSTRING && next[0] == 0x00U) {
		...
		if (stse_certificate->SignatureAlgorithm == SIG_EDDSA_ED25519) {
			...
			if (tag == TAG_SEQUENCE) {
				stse_certificate_parse_integer(next, &stse_certificate->Sign.pR,
							       &stse_certificate->Sign.rSize,
							       &next);
				stse_certificate_parse_integer(next, &stse_certificate->Sign.pS,
							       &stse_certificate->Sign.sSize,
							       &next);
			}
		}
	}
...
#define N_OF_IDENTIFIABLE_SIGNATURE_ALGORITHMS 6
	const struct SignatureAlgorithmOID_st
		signatureAlgorithms_oids[N_OF_IDENTIFIABLE_SIGNATURE_ALGORITHMS] = {
			{.len = 8,
			 .type = SIG_ECDSA_SHA256,
			 .oid = {0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x04, 0x03, 0x02}},
			{.len = 8,
			 .type = SIG_ECDSA_SHA384,
			 .oid = {0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x04, 0x03, 0x03}},
			{.len = 8,
			 .type = SIG_ECDSA_SHA512,
			 .oid = {0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x04, 0x03, 0x04}},
			{.len = 7,
			 .type = SIG_ECDSA_SHA1,
			 .oid = {0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x04, 0x01, 0x00}},
			{.len = 8,
			 .type = SIG_ECDSA_SHA224,
			 .oid = {0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x04, 0x03, 0x01}},
			{.len = 3,
			 .type = SIG_EDDSA_ED25519,
			 .oid = {0x2B, 0x65, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00}},
		};
		...
		if (tag == TAG_OBJECT_IDENTIFIER) {
			for (i = 0; i < N_OF_IDENTIFIABLE_SIGNATURE_ALGORITHMS; i++) {
				if (size == signatureAlgorithms_oids[i].len) {
					if (memcmp(signatureAlgorithms_oids[i].oid, next,
						   (PLAT_UI32)size) == 0) {
						*singatureAlgorithm =
							signatureAlgorithms_oids[i].type;
						break;
					}
				}
			}
...
void stse_certificate_parse_X509_version(const PLAT_UI8 *x509VersionField,
					 stse_certificate_t *stse_certificate,
					 const PLAT_UI8 **next_thing)
{
...
void stse_certificate_parse_ECC_public_key(const PLAT_UI8 *EccPK,
					   stse_certificate_t *stse_certificate,
					   const PLAT_UI8 **next_thing)
{
	PLAT_I32 i, total_size = 0, size = 0, parsed, tag;
...
#define N_OF_IDENTIFIABLE_ECS 10
	const struct EllipticCurveOID_st ellipticCurves_oids[N_OF_IDENTIFIABLE_ECS] = {
		{.len = 8,
		 .type = EC_P256,
		 .oid = {0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x03, 0x01, 0x07, 0}},
		...
		{.len = 9,
		 .type = EC_bp256t1,
		 .oid = {0x2B, 0x24, 0x03, 0x03, 0x02, 0x08, 0x01, 0x01, 0x08}},
		{.len = 9,
		 .type = EC_bp384r1,
		 .oid = {0x2B, 0x24, 0x03, 0x03, 0x02, 0x08, 0x01, 0x01, 11}},
		{.len = 9,
		 .type = EC_bp384t1,
		 .oid = {0x2B, 0x24, 0x03, 0x03, 0x02, 0x08, 0x01, 0x01, 12}},
		{.len = 9,
		 .type = EC_bp512r1,
		 .oid = {0x2B, 0x24, 0x03, 0x03, 0x02, 0x08, 0x01, 0x01, 13}},
		{.len = 9,
		 .type = EC_bp512t1,
		 .oid = {0x2B, 0x24, 0x03, 0x03, 0x02, 0x08, 0x01, 0x01, 14}},
	...
	if (tag == TAG_SEQUENCE) {
		...
		if (tag == TAG_SEQUENCE) {
			tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size, &next);
			if (tag == TAG_OBJECT_IDENTIFIER) {
				if ((memcmp(next, "\x2A\x86\x48\xCE\x3D\x02\x01", 7) == 0) &&
				    (size == 7)) {
					next += 7;
					tag = stse_certificate_identify_ASN1_TLV(next, &parsed,
										 &size, &next);
				}
				if (tag == TAG_OBJECT_IDENTIFIER) {
					for (i = 0; i < N_OF_IDENTIFIABLE_ECS; i++) {
						if (size == ellipticCurves_oids[i].len) {
							if (memcmp(ellipticCurves_oids[i].oid, next,
								   (PLAT_UI32)size) == 0) {
								stse_certificate->EllipticCurve =
									ellipticCurves_oids[i].type;
								break;
							}
						}
					}
					...
					if (tag == TAG_BITSTRING) {
						if (next[0] == 0x00U) {
							...
							switch (next[1]) {
							case 0x04U:
								stse_certificate->PubKey.fsize =
									(size - 2) / 2;
								stse_certificate->PubKey.pX =
									&next[2];
								stse_certificate->PubKey.pY =
									&next[2 + stse_certificate
											  ->PubKey
											  .fsize];
								stse_certificate
									->pPubKey_point_representation_id =
									&next[1];
								break;
							case 0x02U:
							case 0x03U:
...
PLAT_I32 stse_certificate_case_insensitive_compare(const PLAT_UI8 *p1, const PLAT_UI8 *p2,
						   PLAT_I32 size)
{
	PLAT_I32 i; /* For loop*/
	...
	if (tag == TAG_SEQUENCE) {
		...
		if (tag == TAG_OBJECT_IDENTIFIER) {
			...
			if (attribute_st->type != -1) {
				next += size;
				tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &size,
									 &next);
	...
	if (tag == TAG_SEQUENCE) {
		for (i = 0; i < 2; i++) {
			...
			if (tag == TAG_UTCTime || tag == TAG_GeneralizedTime) {
				PLAT_UI32 timevalue;
				if (tag == TAG_UTCTime) {
					timevalue = ((PLAT_UI32)next[0] - 0x30U) * 10U +
						    ((PLAT_UI32)next[1] - 0x30U);
					next += 2;
					size -= 2;
					if (timevalue > 50U) {
					...
					pValidity_st->year = ((PLAT_UI32)next[0] - 0x30U) * 1000U +
							     ((PLAT_UI32)next[1] - 0x30U) * 100U +
							     ((PLAT_UI32)next[2] - 0x30U) * 10U +
							     ((PLAT_UI32)next[3] - 0x30U);
					next += 4;
					size -= 4;
				}
				pValidity_st->month = ((PLAT_UI8)next[0] - 0x30U) * 10U +
						      (PLAT_UI8)next[1] - 0x30U;
				pValidity_st->days = ((PLAT_UI8)next[2] - 0x30U) * 10U +
						     (PLAT_UI8)next[3] - 0x30U;
				pValidity_st->hours = ((PLAT_UI8)next[4] - 0x30U) * 10U +
						      (PLAT_UI8)next[5] - 0x30U;
				pValidity_st->minutes = ((PLAT_UI8)next[6] - 0x30U) * 10U +
							(PLAT_UI8)next[7] - 0x30U;
				pValidity_st->seconds = ((PLAT_UI8)next[8] - 0x30U) * 10U +
							(PLAT_UI8)next[9] - 0x30U;
				next += size;
			}
		}
	}
...
PLAT_I32 stse_certificate_date_compare(const stse_cert_validity_t *D1,
				       const stse_cert_validity_t *D2)
{
	PLAT_I32 test = 0;
...
void stse_certificate_parse_extension(const PLAT_UI8 *ext, PLAT_UI32 *extFlags,
				      stse_cert_extension_t *ext_st)
{
		...
		case EXTENSION_BC:
			...
			*extFlags |= 1U; /* BC is present */
			...
			if (tag == TAG_OCTETSTRING) {
				...
				tag = stse_certificate_identify_ASN1_TLV(next, &parsed, &seqSize,
									 &next);
				...
				if (tag == TAG_SEQUENCE) {
					...
					{
						tag = stse_certificate_identify_ASN1_TLV(
							next, &parsed, &size, &next);
						...
						if (next <
						    ext_st->value +
							    ext_st->valueSize) { /* Yes, then it's
										    the path */
							tag = stse_certificate_identify_ASN1_TLV(
								next, &parsed, &size, &next);
							if (tag == TAG_INTEGER) {
								...
								*extFlags |=
									((PLAT_UI32)
										 stse_certificate_get_small_integer(
											 next,
											 size) &
									 15U)
									<< 4; /* 4-bit encoding for
										 pathLen */
							}
						}
					}
...
#define MAX(a, b) (((a) > (b)) ? (a) : (b))
					/* This reads the 8 bits (or less) and put it in the right
					 * place */
					for (i = 0; i < MAX((PLAT_I32)next[0], 8); i++) {
						/* Not very readable */
						*extFlags |= (((PLAT_UI32)next[1] >>
							       (PLAT_UI8)(7U - (PLAT_UI8)i)) &
							      1U)
							     << (PLAT_UI8)(16U + (PLAT_UI8)i);
					}
					if (next[0] == 9U && seqSize == 2) {
						*extFlags |= (((PLAT_UI32)next[2] >> 7) & 1U)
							     << (15);
					}
#undef MAX
				}
