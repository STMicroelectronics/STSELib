						    ...
						    PLAT_I32 signatureSsize)
{
		...
		if (*cert->pPubKey_point_representation_id == 0x04) {
			...
			if (stsafe_x509_parser_companion_handler != NULL) {
				stsafea_ecc_decompress_public_key(
					stsafe_x509_parser_companion_handler, key_type,
					*cert->pPubKey_point_representation_id, pub_key,
	...
	if (stsafe_x509_parser_companion_handler != NULL) {
		...
		ret = stse_ecc_verify_signature(stsafe_x509_parser_companion_handler, key_type,
						pub_key, signature, (PLAT_UI8 *)digest,
						(PLAT_UI16)digestSize, 1, /* Message is hashed */
						&signature_validity);
...
#endif
	{
		ret = stse_platform_ecc_verify(key_type, pub_key, (PLAT_UI8 *)digest,
					       (PLAT_UI16)digestSize, signature);
