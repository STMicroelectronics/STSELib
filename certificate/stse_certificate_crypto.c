...
stse_ReturnCode_t stse_certificate_verify_cert_signature(const stse_certificate_t *parent,
                                                         const stse_certificate_t *child)
{
  ...
  /* Ed25519 verifies over the raw TBS message bytes, while ECDSA paths
   * verify over a digest computed with the signature-declared hash algorithm. */
...
#endif /* defined(STSE_CONF_USE_COMPANION) */
  else
  {
    /* Fallback to host platform hashing when no compatible companion is used. */
                                                    ...
                                                    PLAT_I32 signatureSsize)
{
  ...
  /* Rebuild the verification public key as an uncompressed X||Y buffer.
   * When the certificate stores a compressed point, request decompression
   * from a companion secure element before signature verification. */
  ...
  if (stsafe_x509_parser_companion_handler != NULL)
  {
    /* Ask the companion to verify signature material in secure hardware. */
...
#endif /* defined(STSE_CONF_USE_COMPANION) */
  {
    /* Host-software fallback for targets without companion acceleration. */
...
{
  /* Map parsed certificate curve identifiers to middleware ECC key-type indices.
   * These indices are later used to size public keys/signatures consistently. */
...
{
  /* Signature algorithm OID already parsed from certificate is converted to the
   * hash selector expected by host/companion hashing implementations. */
