...
{
	PLAT_I32 i = 0;
	/* The first byte tells us if it's positive or negative. We don't support
	 * negative */
	if (value[0] >> 7 == 1) {
...
void stse_certificate_print_parsed_cert(stse_certificate_t *stse_certificate)
{
	...
	if (stse_certificate->serialNumber != NULL && stse_certificate->serialNumberSize > 0) {
		print_buffer("\n\r\tSerialNumber: ", stse_certificate->serialNumber,
	...
	if (stse_certificate->subject != NULL && stse_certificate->subjectSize > 0) {
		printf("\n\r\t Subject:");
		stse_certificate_print_name(stse_certificate->subject,
					    stse_certificate->subjectSize);
	}
	...
	if (tag == TAG_SET) {
		...
		if (tag == TAG_SEQUENCE) {
			...
			if (tag == TAG_OBJECT_IDENTIFIER) {
				attribute_st->type =
					stse_certificate_identify_attribute(next, size);
...
void stse_certificate_print_name(const PLAT_UI8 *name, PLAT_I32 nameSize)
{
	...
	if (tag == TAG_SEQUENCE) {
		...
		while (next < name + nameSize) {
			...
			} else if (attribute_st.type != -1 &&
				   attribute_st.strFormat == TAG_BITSTRING) {
				stse_certificate_print_bit_string(attribute_st.str,
								  attribute_st.strSize);
			}
		}
	}
}
...
void stse_certificate_print_validity(const PLAT_UI8 *validity)
{
	...
	printf("\n\r\t Validity:");
	printf("\n\r\t\t Not Before: %04" PRId32 "-%02d-%02d %02d:%02d:%02d", notBefore_st.year,
	       notBefore_st.month, notBefore_st.days, notBefore_st.hours, notBefore_st.minutes,
	       notBefore_st.seconds);
	printf("\n\r\t\t Not After:  %04" PRId32 "-%02d-%02d %02d:%02d:%02d", notAfter_st.year,
	       notAfter_st.month, notAfter_st.days, notAfter_st.hours, notAfter_st.minutes,
	       notAfter_st.seconds);
}
