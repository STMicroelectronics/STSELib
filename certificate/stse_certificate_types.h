...
typedef struct extension_st {
	...
	const PLAT_UI8 *value; /*!< pointer to the value hold by the extension (this
				  is an octet string)  */
...
struct stse_internal_certificate {
	const PLAT_UI8 *pPubKey_point_representation_id; /*!< 0x04: Uncompressed key, 0x02 or
							    0x03: Compressed key */
	...
	PLAT_UI32 extensionsFlags;     /*!<  Integer encoding the fields present in the
					*tcs:			\n   bit 0:
					*BasicConstraints is present
					*\n   bit 1:   BasicConstraints is critical
					*\n   bit 2:   BasicConstraints indicates this is
					*a   CA			\n   bit 3:
					*BasicConstraints has a pathLenConstraint
					*\n   bit 4-7: PathLen
					...
					*\n   bit 17:  keyUsage field
					*contentCommitment/nonRepudiation	\n   bit 18:
					*keyUsage field keyEncipherment
...
#define IS_VALID_TAG(tag)                                                                          \
	(((TAG_BOOLEAN <= (tag) && (tag) <= TAG_ObjectDescriptor) || (tag) == TAG_ENUMERATED ||    \
	  (tag) == TAG_UTF8String || (tag) == TAG_PrintableString ||                               \
	  (TAG_IA5String <= (tag) && (tag) <= TAG_GeneralizedTime) || (tag) == TAG_SEQUENCE ||     \
	  (tag) == TAG_SET || (TAG_x509VERSION <= (tag) && (tag) <= TAG_extensions))               \
		 ? 1                                                                               \
		 : 0)
