...
typedef enum
{
  ...
  TAG_OBJECT_DESCRIPTOR,       /*!< x509 tag for ObjectDescriptor */
  TAG_ENUMERATED = 0x0A,      /*!< x509 tag for ENUMERATED */
  TAG_UTF8_STRING = 0x0C,      /*!< x509 tag for UTF8String */
  TAG_PRINTABLE_STRING = 0x13, /*!< x509 tag for PrintableString */
  TAG_IA5_STRING = 0x16,       /*!< x509 tag for IA5String */
  TAG_UTC_TIME,                /*!< x509 tag for UTCTime */
  TAG_GENERALIZED_TIME,        /*!< x509 tag for GeneralizedTime */
  TAG_SEQUENCE = 0x30,        /*!< x509 tag for SEQUENCE */
  TAG_SET,                    /*!< x509 tag for SET */
  TAG_X509_VERSION = 0xA0,     /*!< x509 tag for x509VERSION */
  TAG_ISSUER_UNIQUE_ID,         /*!< x509 tag for issuerUniqueID */
  TAG_SUBJECT_UNIQUE_ID,        /*!< x509 tag for subjectUniqueID */
...
typedef struct stse_cert_public_key_t
{
  ...
  const PLAT_UI8 *pY; /*!<  Pointer to Y Coordinate */
...
#define IS_VALID_TAG(tag) ((                                                               \
                           (TAG_BOOLEAN <= (tag) && (tag) <= TAG_OBJECT_DESCRIPTOR) ||  \
                           (tag) == TAG_ENUMERATED ||                                  \
                           (tag) == TAG_UTF8_STRING ||                                  \
                           (tag) == TAG_PRINTABLE_STRING ||                             \
                           (TAG_IA5_STRING <= (tag) && (tag) <= TAG_GENERALIZED_TIME) || \
                           (tag) == TAG_SEQUENCE ||                                    \
                           (tag) == TAG_SET ||                                         \
                           (TAG_X509_VERSION <= (tag) && (tag) <= TAG_EXTENSIONS))      \
                           ? 1                                                         \
                           : 0)
