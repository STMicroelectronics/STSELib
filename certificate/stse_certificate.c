...
static void parsetbsCertificate(const PLAT_UI8 *tbs, PLAT_I32 tbsSize, stse_certificate_t *stse_certificate)
{
  ...
  if (tbs != NULL && stse_certificate != NULL)
  {
    ...
    if (tag == TAG_SEQUENCE)
    {
      ...
      stse_certificate_parse_integer(next_value, &stse_certificate->serialNumber, &stse_certificate->serialNumberSize,
                                     &next_value);
      /* Parse tbsSignature(Algorithm)*/
      ...
      if (next_value < tbs + tbsSize)
      {
        ...
        if (tag == TAG_EXTENSIONS)
        {
          ...
          stse_certificate_parse_extensions(stse_certificate->extensions,
                                            stse_certificate->extensionsSize,
                                            stse_certificate);
        }
      }
    }
                                         ...
                                         const PLAT_UI8 **next)
{
  ...
  /* Now the Signature is available. If it's ECDSA parse it! */
...
stse_ReturnCode_t stse_certificate_is_parent(const stse_certificate_t *parent, const stse_certificate_t *child,
                                             const stse_cert_validity_t *currentTime)
{
  ...
  {
    /* Parse RDN1 and RDN2. The SEQUENCE is ordered, so this must match */
    ...
    while (attr1 < name1) /*Loop until the next RDN1 */
    {
      ...
      do                  /* Scans the whole DN SET in Name2 */
      {
        ...
        if (attr1_st.type == attr2_st.type)
        {
          ...
          if (attr1_st.strFormat == TAG_PRINTABLE_STRING || attr1_st.strFormat == TAG_UTF8_STRING)
          {
            /* Then attr2 must be of string type with the same size */
            if ((attr2_st.strFormat != TAG_PRINTABLE_STRING && attr2_st.strFormat != TAG_UTF8_STRING)
                || attr1_st.strSize != attr2_st.strSize)
            {
              return (STSE_CERT_CA_NOT_MATCHING);
            }
            /* Now they are both string type and of the same size, they must match */
          ...
          /* attributes must be of the same type and size */
