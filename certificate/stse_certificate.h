...
/**
  ...
  * \note  This function must be called before parsing a certificate
  */
...
/**
  ...
  * \param[in]  currentTime      pointer to a stse_cert_validity_t with the
  *                              current DateTime. If NULL no date check is done
  * \return \ref STSE_OK on success ; \ref stse_ReturnCode_t error code otherwise
...
/**
  ...
  *     As result of this simplification some legitimate parentship relation
  *     could be not acknowledged (the opposite must not occur).
