...
#define stse_frame_strap_allocate(strap) stse_frame_element_t strap = {0, NULL, NULL};
...
/**
 * \brief 			Attach a strap element that reroute a frame
 * element (Element1) to another (Element2)
 * \details 		This core function attach a strap element that link
 * Element1 to Element2 until un-strap command is executed \param[in]
 * pStrap 			Pointer to strap element \param[in]
 * pElement_1 		Pointer to frame element 1 \param[in] pElement_2
 * Pointer to frame element 2
 */
...
/**
 * \brief 			swap the Data byte order pointed by pData frame
 * element \details 		This core function swap the Data byte order
 * pointed/defined by pData and length value from frame \param[in]
 * pElement 			Pointer to frame element
 */
void stse_frame_element_swap_byte_order(stse_frame_element_t *pElement);

/**
 * \brief 			Push frame element into frame
 * \details 		This core function set selected element as the frame
 * last one \param[in] 		pFrame 				Pointer to frame
